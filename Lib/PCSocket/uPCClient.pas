...
type
  ...
  private
    ...
    procedure RcvDeviceFTPPercent(aNodeNo,aEcuID,aData:string); //FTP 퍼센트
  ...
  private
    ...
    FServerName: string;
    FOnFTPPercent: TDeviceChangeEvent;
  ...
  published
    ...
    property OnFTPPercent : TDeviceChangeEvent read FOnFTPPercent write FOnFTPPercent;
...
begin
  ...
  if (stSubCLass = 'MN') and (stAlarmStateCode = 'NR')  then bServerAlarm := True; //복구 신호 이벤트는 서버에서 생성한 이벤트이다.
...
procedure TdmPCClient.RcvDeviceFTPPercent(aNodeNo, aEcuID, aData: string);
var
  nIndex : integer;
begin
  if aData = '' then Exit;
  if Assigned(FOnFTPPercent) then
  begin
    OnFTPPercent(Self,DecoderNo,aNodeNo,aEcuID,'0','0','0',aData);
  end;
...
begin
  ...
  case aRealData[3] of
    ...
    'F' : begin  //FTP 게이지 퍼센트
        stData := aRealData;
        Delete(stData,1,4 + G_nNodeCodeLength + 1);
        Delete(stData,1,3);    //aExtendID[1] + FillZeroNumber(strtoint(aNumber),2)
        RcvDeviceFTPPercent(stNodeNo,stEcuID,stData);
    end;
