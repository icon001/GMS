...
begin
  ...
  begin
    if strtoint(Value) = 0  then  //안티패스백을 사용하지 않는다.
    begin
      if Not ((CARDMODE='') or (CARDMODE='0') or (CARDMODE = '1')) then  //Positive 또는 Negative 모드가 아니면 Positive 모드로 변경하자.
      begin
        stData := 'C' +
                  '0' +  //  Msg Count
                  inttostr(DoorNo) +
    ...
    begin
      if ((CARDMODE='0') or (CARDMODE = '1')) then  //Positive 또는 Negative 모드이면 Positive-ACK 모드로 변경하자.
      begin
        stData := 'C' +
                  '0' +  //  Msg Count
                  inttostr(DoorNo) +
  ...
  begin
    if Not ((CARDMODE='0') or (CARDMODE = '1')) then  //Positive 또는 Negative 모드가 아니면 Positive 모드로 변경하자.
    begin
      stData := 'C' +
                '0' +  //  Msg Count
                inttostr(DoorNo) +
...
begin
  ...
  begin
    ...
    begin
      if strtoint(AntiGroupCode) = 0  then  //안티패스백을 사용하지 않는다.
      begin
        if Not ((CARDMODE='0') or (CARDMODE = '1')) then  //Positive 또는 Negative 모드가 아니면 Positive 모드로 변경하자.
        begin
          stData := 'C' +
