...
//  G_bDoorLongTimeOpenAlarmEventUse : Boolean = True; //장시간 열림 발생시 이벤트 발생 - 데몬에서 처리
  ...
  G_nAttendRelayFormat : integer = 0;     //근태연동포맷
  G_nAutoLogOutTime : integer = 1;      //자동로그아웃시간(분)
  G_nBMSType : integer = 2;        //2.출입,3.근태,4.식수,5.기타
  ...
  G_nCheckFingerCardPos : integer = 0; //지문카드인지 아닌지 체크 위치
  ...
  G_nDefaultExtentionCount : integer = 1;  //초기생성 디폴트 컨트롤러확장기 갯수
  ...
  G_nFingerReaderType : integer = 2; //1.성현지문리더,2.슈프리마지문리더
  G_nFingerRelayDBType : integer = 1; //1.MSSQL
  G_nFingerRelayType : integer = 1; //1.TCPIP,2.DataBase 연동,3.Zeron TCP/IP  지문리더 데이터 전송 방식
  G_nFingerServerPort : integer = 7006; //
  G_nFPUserCardLength : integer = 6; //모국 카드번호는 6자리로 만들어야 한다. 나머지는 4자리
...
//  G_nMultiSocket : integer = 0;
  ...
  G_nMonitoringType : integer = 0;  //0.일반 타입,1.분기국사 타입,2.SK/SG
  ...
  G_nProgramType : integer = 0; //0 : KTTelecop,1:S-TEC,2:SKT,3.SG생활안전
  G_nReportFileFormat : integer = 0; //0.Excel,1.CSV
  G_nReportCompanyStep : integer = 3; //보고서 조회시 소속 단계
  G_nReportSearchType : integer = 1; // 보고서 조회시 1.바로검색,2.검색버튼 클릭시 조회
  G_nScheduleDevice : integer;    //스케줄 0:컨트롤러자체동작,1:PC스케줄
  G_nShFingerRegDevicePort : integer = 7005; //성현지문리더 등록타입 포트
  G_nShFingerRegDeviceID : integer = 1; //성현지문리더 등록타입 아이디
  G_nShFingerRegType : integer = 0; //성현지문리더 등록기타입  0.지문등록기,1.지문리더기
...
//  G_stDaemonFTPPort : string;
  ...
  G_stShFingerRegDeviceIP : string = '127.0.0.1'; //성현지문리더 등록타입 아이디
  ...
  FingerReaderList : TStringList; //지문리더 리스트
