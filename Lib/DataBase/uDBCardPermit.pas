...
type
  ...
  public
    ...
    Function EmGroupArmAreaPermitLikeAdd(aEmGroupCode,aNodeNo,aEcuID,aExtendID,aArmAreaNo:string):Boolean; //그룹코드별 방범권한
    Function EmGroupArmAreaPermitLikeDelete(aEmGroupCode,aNodeNo,aEcuID,aExtendID,aArmAreaNo:string):Boolean;
    ...
    Function EmGroupGradeToEmployeeGradeLike(aEmGroupCode:string=''):Boolean;
...
begin
  ...
  begin
    ...
    stSql := stSql + ' WHERE A.GROUP_CODE = ''' + G_stGroupCode + ''' ';
    stSql := stSql + ' AND EM_GRADETYPE = 1 '; //권한 타입이 회사코드 타입인 경우만
  ...
  stSql := stSql + ' AND aa.EM_GRADETYPE = 1 '; //권한 타입이 회사코드 타입인 경우만
  stSql := stSql + ' AND aa.EM_ACUSE = ''1'' ';      //출입자에서 삭제 되지 않은 경우
...
function TdmDBCardPermit.EmGroupArmAreaPermitLikeAdd(aEmGroupCode, aNodeNo,
  aEcuID, aExtendID, aArmAreaNo: string): Boolean;
begin
  //그룹에 권한이 있으면서 틀린 경우 업데이트 하자.
  dmDBUpdate.UpdateTB_CARDPERMITEMPLOYEEGROUP_LikeEmGroupArmAreaPermitAdd(aEmGroupCode, aNodeNo, aEcuID,aExtendID, aArmAreaNo);
  //그룹에 권한이 없는 경우 Insert 하자.
  dmDBInsert.InsertIntoTB_CARDPERMITEMPLOYEEGROUP_LikeEmGroupArmAreaPermitAdd(aEmGroupCode, aNodeNo, aEcuID,aExtendID, aArmAreaNo);

end;

function TdmDBCardPermit.EmGroupArmAreaPermitLikeDelete(aEmGroupCode, aNodeNo,
  aEcuID, aExtendID, aArmAreaNo: string): Boolean;
begin
  //그룹에 권한이 있으면서 틀린 경우 업데이트 하자.
  dmDBUpdate.UpdateTB_CARDPERMITEMPLOYEEGROUP_LikeEmGroupArmAreaPermitDelete(aEmGroupCode, aNodeNo, aEcuID,aExtendID, aArmAreaNo);
...
function TdmDBCardPermit.EmGroupGradeToEmployeeGradeLike(
  aEmGroupCode: string): Boolean;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
