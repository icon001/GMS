...
procedure TdmFTPModule.DataModuleCreate(Sender: TObject);
begin
  IdFTPServer := tIdFTPServer.create( nil ) ;
end;

procedure TdmFTPModule.DataModuleDestroy(Sender: TObject);
begin
...
function TdmFTPModule.GetSizeOfFile(AFile: String): integer;
var
  FStream : TFileStream;
begin
  Try
...
Var
 LFile : String;
begin
 LFile := ReplaceChars( ASender.HomeDir + '/' + AFilename );
 try
...
function TdmFTPModule.ReplaceChars(APath: String):string;
var
 s:string;
begin
  s := StringReplace(APath, '/', '\', [rfReplaceAll]); //字符串替换函数
  s := StringReplace(s, '\\', '\', [rfReplaceAll]);
  Result := s;
end;
