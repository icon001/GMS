...
type
  ...
  TdmDirectServer = class(TDataModule)
...
var
  dmDirectServer: TdmDirectServer;
...
procedure TdmDirectServer.AlarmConfirm;
var
  stSendData : string;
  i : integer;
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.AlarmConfirm');
  End;
end;

procedure TdmDirectServer.ArmAreaModeChange(aNodeNo: integer; aEcuID,
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.ArmAreaModeChange');
  End;
end;

procedure TdmDirectServer.ClientPacketView(Sender: TObject; aClientIP,
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.ClientPacketView');
  End;
end;

procedure TdmDirectServer.ClientSocketConnectCheckTimerTimer(
  Sender: TObject);
var
  i : integer;
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.ClientSocketConnectCheckTimerTimer');
  End;
end;

procedure TdmDirectServer.CurrentAlarmEvent(aNodeNo, aECUID,aExtendID, aCmd, aMsgNo,
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.CurrentAlarmEvent');
  End;
end;

procedure TdmDirectServer.CurrentDeviceConnectedChange(aNodeNo:integer; aEcuID,
  stConnected: string);
var
  stSendData : string;
  i : integer;
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.CurrentDeviceConnectedChange');
  End;
end;

procedure TdmDirectServer.CurrentDoorState(aNodeNo: integer; aEcuID,
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.CurrentDoorState');
  End;
end;

procedure TdmDirectServer.DataModuleCreate(Sender: TObject);
begin
  Try
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DataModuleCreate');
  End;
end;

procedure TdmDirectServer.DataModuleDestroy(Sender: TObject);
var
  i : integer;
begin
  Try
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DataModuleDestroy');
  End;
end;

procedure TdmDirectServer.DBSocketClear(aSeq: integer);
var
  stSql : string;
begin
  Try
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DBSocketClear');
  End;
end;

procedure TdmDirectServer.DBSocketInitialize;
var
  stSql : string;
begin
  Try
    ...
    dmDataBase.ProcessExecSQL(stSql);
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DBSocketInitialize');
  End;
end;


procedure TdmDirectServer.DeamonRestart(Sender: TObject);
begin
  Try
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DeamonRestart');
  End;
end;

procedure TdmDirectServer.DeviceCardAccessEvent(aNodeNo, aECUID, aDoorNo,
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.DeviceCardAccessEvent');
  End;
end;

procedure TdmDirectServer.FTPCardDownLoadPercentChange(aNodeNo, aEcuID,
  aExtendID, aNumber, aData: string);
var
  stSendData : string;
  i : integer;
...
function TdmDirectServer.GetHandle: THandle;
begin
  Try
    HandleNeeded;
    Result := FHandle;
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.GetHandle');
  End;
end;

function TdmDirectServer.GetLastDBSocketSerial: integer;
var
  stSql : string;
...
begin
  Try
    ...
    End;
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.GetLastDBSocketSerial');
  End;
end;

function TdmDirectServer.HandleAllocated: Boolean;
begin
  Try
    Result := ( FHandle <> 0 );
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.HandleAllocated');
  End;
end;

procedure TdmDirectServer.HandleNeeded;
begin
  Try
    if not HandleAllocated
     then FHandle := AllocateHWND ( WndProc );
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.HandleNeeded');
  End;
end;

procedure TdmDirectServer.handle_wm_async_select(var Msg: TMessage);
  ...
  begin
    ...
    Except
      LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.handle_fd_accept_notification');
    End;
  end;
  ...
  begin
    ...
    Except
      LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.handle_fd_write_notification');
    End;
  end;
  ...
  begin
    ...
    Except
      LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.handle_fd_read_notification');
    End;
  end;
  ...
  begin
    ...
    Except
      LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.handle_fd_close_notification');
    End;
  end;
var l_param: Integer;
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.handle_wm_async_select');
  End;
end;

procedure TdmDirectServer.ServerNodeConnected(aNodeNo: string);
var
  i : integer;
begin
  Try
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.ServerNodeConnected');
  End;
end;

procedure TdmDirectServer.SetServerStart(const Value: Boolean);
var
  l_result : integer;
...
begin
  ...
  Except
    LogSave(G_stLogDirectory + '\err' + FormatDateTime('yyyymmdd',now) + '.log','dmDirectServer.SetServerStart');
  End;
end;


procedure TdmDirectServer.SocketClose(Sender: TObject; aSocketNo: integer);
var
  nIndex : integer;
begin
...
procedure TdmDirectServer.SocketConnected(Sender: TObject;
  aSocketNo: integer);
begin
...
procedure TdmDirectServer.WndProc(var Message: TMessage);
begin
  Dispatch ( Message );
end;
  ...
  Try
    ...
    case aRealData[3] of
      'D' : begin  //출입문 상태
        stDoorNo := copy(aRealData,4 + G_nNodeCodeLength + 2,2);
        if Not isDigit(stDoorNo) then Exit;
        if strtoint(stDoorNo) = 0 then Exit; //0번 출입문은 조회할 필요 없다.
