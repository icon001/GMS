...
uses
  ...
  uMapDoorState, CommandArray, AdvPageControl, AdvToolBar, AdvToolBarStylers,
...
begin
  inherited;
  if aChildName = 'TfmMapDoorState' then
  begin
...
begin
  ...
  begin;
    TfmMapDoorState(BuildingFormList.Objects[i]).ParentLeft := Value;
  end;
...
begin
  ...
  for i := 0 to BuildingFormList.Count - 1 do
  begin;
    TfmMapDoorState(BuildingFormList.Objects[i]).ParentTop := Value;
  end;
end;
...
var
  i : integer;
  DockWindow : TfmMapDoorState;
  DockTapSheet : TTabSheet;
begin
  for i := 0 to BuildingFormList.Count - 1 do
  begin;
    DockWindow := TfmMapDoorState(BuildingFormList.Objects[i]);
...
begin
  ...
  begin
    ...
    BuildingFormList.AddObject(stBuildingCode,oDoorMonitorForm);
  end;
end;
...
begin
  ...
  for i := 0 to BuildingFormList.Count - 1 do
    TfmMapDoorState(BuildingFormList.Objects[i]).DeviceReload;
end;
...
begin
  ...
  if nIndex > -1 then
  begin
    TfmMapDoorState(BuildingFormList.Objects[nIndex]).DoorChangeEvent(aNodeNo, aEcuID, aType, aNo,aCmd, aData,aBuildingCode);
  end;
...
procedure TfmMapDoorMonitoring.FontSetting;
begin
...
begin
  ...
  begin
    for i := BuildingFormList.Count - 1 downto 0 do
    begin
      TfmMapDoorState(BuildingFormList.Objects[i]).Close;
      TfmMapDoorState(BuildingFormList.Objects[i]).Free;
    end;
  end;
