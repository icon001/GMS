...
uses
  ...
  ADODB,ActiveX,AdvCombo,Data.DB,Vcl.Imaging.jpeg,Vcl.Menus;
...
type
  ...
  public
    ...
    Function TravelPopupMenuSubItem(aMenu:TPopupMenu;stName:string; no:Integer):TMenuItem;
    Function TravelMenuSubItem(aMenu:TMenuItem;stName:string; no:Integer):TMenuItem;
    //*************************
...
procedure TdmFormFunction.JPEGLoadFromDB(Field: TField; Image: TImage);
var
  jpg: TJPEGImage;
  mem: TMemoryStream;
...
function TdmFormFunction.TravelMenuSubItem(aMenu: TMenuItem; stName: string;
  no: Integer): TMenuItem;
var
Loop:integer;
begin
  ...
  For Loop:=0 to aMenu.Count -1 do
  Begin
    If LowerCase(aMenu[Loop].Name) = LowerCase(stName) + inttostr(no) then
    Begin
      Result:=TMenuItem(aMenu[Loop]);
      exit;
    End;
  End;
end;
...
function TdmFormFunction.TravelPopupMenuSubItem(aMenu: TPopupMenu;
  stName: string; no: Integer): TMenuItem;
var
Loop:integer;
begin
  ...
  For Loop:=0 to aMenu.Items.Count -1 do
  Begin
    If LowerCase(aMenu.Items[Loop].Name) = LowerCase(stName) + inttostr(no) then
    Begin
      Result:=TMenuItem(aMenu.Items[Loop]);
      exit;
    End;
  End;
