...
begin
  ...
  DisplayList.Add(aClientIP + ',' + aTxRx + ',' + aResult + ',' + aData );
end;
...
begin
  Caption := dmFormFunction.GetFormName('2','1','BM3_015');
  AdvOfficePage1.Caption := dmFormFunction.GetFormName('2','1','BM3_015');
  lb_List.Caption := dmFormFunction.GetFormName('2','2','BM3_015List');
...
procedure TfmPCComMonitoring.SearchTimerTimer(Sender: TObject);
var
  stClientIP : string;
  stTxRx : string;
  stResult : string;
...
begin
  ...
  stClientIP := FindCharCopy(stTemp,0,',');
  stTxRx := FindCharCopy(stTemp,1,',');
  stResult := FindCharCopy(stTemp,2,',');
  ...
  begin
    ...
    if UpperCase(stResult) = 'ER' then RowColor[1] := clYellow;
