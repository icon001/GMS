...
type
  ...
  public
    ...
    function ACAMTB_Card_OldCardDelete : Boolean;
...
function TForm1.ACAMTB_Card_OldCardDelete: Boolean;
var
  stSql : string;
begin
  stSql := 'Delete From TB_CARD where CA_CARDGUBUN = ''2'' ';
...
begin
  if Not DBConnect then Exit;
  if Not SKDBConnect then Exit;
  if ACAMTB_Card_OldCardDelete then
  begin
    showmessage('삭제 완료');
  end else
  begin
...
function TForm1.UpdateTB_CONFIG_Field_StringValue(aGroup, aCode, aFieldName,
  aData: string): Boolean;
var
  stSql : string;
begin
