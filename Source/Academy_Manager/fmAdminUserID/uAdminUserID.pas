...
type
  TfmAdminUserID = class(TfmASubForm)
    ...
    procedure AddMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
...
procedure TfmAdminUserID.AddMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TfmAdminUserID.AddMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
const
 SC_DragMove = $F012;
begin
...
procedure TfmAdminUserID.AddMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
(*    // MOUSE DOWN = FALSE 설정
...
begin
  ...
  sg_List.Cells[3,0] := '핸드폰번호';
  sg_List.Cells[4,0] := '이메일';
  sg_List.Cells[5,0] := '기타';
...
begin
  ...
  sg_List.ColWidths[5] := sg_List.Width - (sg_List.ColWidths[0] + sg_List.ColWidths[1] + sg_List.ColWidths[2] + sg_List.ColWidths[3] + sg_List.ColWidths[4]);
  if sg_List.ColWidths[5] < 100 then sg_List.ColWidths[5] := 100;
...
*)
  ...
  Try
    ...
    begin
      ...
      ed_email.Text := cells[4,Row];
      ed_etc.Text := cells[5,Row];
      ed_AdminPassword.Text := cells[12,Row];
      if cells[7,Row] = '1' then
      begin
      ...
      ed_CompanyCode.Text := cells[8,Row];
      vNode1:= GetNodeByText(tv_AddCompanyCode,cells[8,Row],True);
      if vNode1 <> nil then
      begin
