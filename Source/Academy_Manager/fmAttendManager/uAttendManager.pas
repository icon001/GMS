...
(*  if DayOfWeek(Now) = 1 then stSql := stSql + ' AND c.CC_SUN = ''1'' '
  ...
  if DayOfWeek(Now) = 1 then stSql := stSql + ' AND b.EM_ACSUN = ''1'' '
  else if DayOfWeek(Now) = 2 then stSql := stSql + ' AND b.EM_ACMON = ''1'' '
  else if DayOfWeek(Now) = 3 then stSql := stSql + ' AND b.EM_ACTUE = ''1'' '
  else if DayOfWeek(Now) = 4 then stSql := stSql + ' AND b.EM_ACWED = ''1'' '
  else if DayOfWeek(Now) = 5 then stSql := stSql + ' AND b.EM_ACTHU = ''1'' '
  else if DayOfWeek(Now) = 6 then stSql := stSql + ' AND b.EM_ACFRI = ''1'' '
  else if DayOfWeek(Now) = 7 then stSql := stSql + ' AND b.EM_ACSAT = ''1'' ';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dmDBUpdate.UpdateTB_ACademyAttendSummary_Field_StringValue(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'AS_INTIME','');
      end else
      begin
        dmDBInsert.InsertIntoTB_ACademyAttendSummary_Value(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'','','0');
      end;
    ...
    begin
      ...
      begin
        ...
        dmDBUpdate.UpdateTB_ACademyAttendSummary_Field_StringValue(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'AS_INTIME',FormatDateTime('hhnnss',now));
      end else
      begin
        dmDBInsert.InsertIntoTB_ACademyAttendSummary_Value(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],FormatDateTime('hhnnss',now),'','1');
      end;
  ...
  begin
    ...
    begin
      ...
      if dmDBFunction.CheckTB_ACademyAttendSummary_Key(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow]) = 1 then
      begin
        dmDBUpdate.UpdateTB_ACademyAttendSummary_Field_StringValue(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'AS_OUTTIME','');
      end else
      begin
        dmDBInsert.InsertIntoTB_ACademyAttendSummary_Value(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'','','0');  //여기로 들어오면 문제 있는것임
      end;
    ...
    begin
      ...
      if dmDBFunction.CheckTB_ACademyAttendSummary_Key(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow]) = 1 then
      begin
        dmDBUpdate.UpdateTB_ACademyAttendSummary_Field_StringValue(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'AS_OUTTIME',FormatDateTime('hhnnss',now));
        dmDBUpdate.UpdateTB_ACademyAttendSummary_Field_StringValue(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'AS_UPDATETIME',FormatDateTime('yyyymmddhhnnsszzz',now));
      end else
      begin
        dmDBInsert.InsertIntoTB_ACademyAttendSummary_Value(FormatDateTime('yyyymmdd',now),sg_EmployeeList.Cells[10,ARow],'','','0');  //여기로 들어오면 문제 있는것임
      end;
