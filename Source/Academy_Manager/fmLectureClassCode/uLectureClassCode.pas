...
type
  ...
  private
    ...
    L_stWork : string;
    L_stSelectLectureClassCode : string;
...
begin
  ...
  begin
    ...
    LoadLectureClassTreeView(stParentCode,TreeView_Select,tv_emGroupCode,0);
...
procedure TfmLectureClassCode.btn_SelectCancelClick(Sender: TObject);
begin
  inherited;
  pan_SelectViewSeq.Visible := False;
end;

procedure TfmLectureClassCode.btn_SelectSaveClick(Sender: TObject);
var
  stTargetCode : string;
  stViewSeq : string;
begin
  ...
  stViewSeq := dmDBFunction.GetTB_LectureClassCode_ViewSeq(stTargetCode);
  if isDigit(stViewSeq) then
  begin
    dmDBUpdate.UpdateTB_LectureClassCode_ViewSeqUp(stViewSeq);
    dmDBUpdate.UpdateTB_LectureClassCode_Field_IntValue(L_stSelectLectureClassCode,'LC_VIEWSEQ',stViewSeq);
  end;
  pan_SelectViewSeq.Visible := False;
  LoadLectureClassTreeView('',tv_emGroupList,tv_emGroupCode,0);
  LoadLectureClassTreeView('',TreeView_Select,tv_emGroupCode,0);
end;
...
procedure TfmLectureClassCode.mn_ViewSeqChangeClick(Sender: TObject);
begin
  inherited;
  L_stSelectLectureClassCode := tv_emGroupCode.Items[tv_emGroupList.Selected.AbsoluteIndex].Text;
...
*)
  ...
  LoadLectureClassTreeView(stCode,TreeView_Select,tv_emGroupCode,0);
end;
