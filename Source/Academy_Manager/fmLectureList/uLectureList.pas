...
type
  TfmLectureList = class(TfmASubForm)
    ...
    chk_CurentEmployee: TAdvOfficeCheckBox;
    ...
    procedure ed_SearchPhoneKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure chk_CurentEmployeeClick(Sender: TObject);
  private
    DragOrigin: TPoint;
...
procedure TfmLectureList.chk_CurentEmployeeClick(Sender: TObject);
begin
  inherited;
  SearchEmployeeList('');
end;
...
begin
  ...
  chk_CurentEmployee.Checked := True;
...
begin
  ...
  if cmb_CurriculumCode.ItemIndex > -1 then stLectureCode := CurriculumCodelist.Strings[cmb_CurriculumCode.ItemIndex] ;
  ...
  if ed_SerchSelectLectureClassCode.Text <> '' then stSql := stSql + ' AND d.LC_CODE Like ''' + ed_SerchSelectLectureClassCode.Text + '%'' ';
  if ed_SerchSelectCompanyCode.Text <> '' then stSql := stSql + ' AND b.CO_COMPANYCODE Like ''' + ed_SerchSelectCompanyCode.Text + '%'' ';
  if stLectureCode <> '' then stSql := stSql + ' AND a.CC_CODE = ' + stLectureCode + ' ';
  ...
  if chk_CurentEmployee.Checked then stSql := stSql + ' AND a.EL_STATE = 1 ';
