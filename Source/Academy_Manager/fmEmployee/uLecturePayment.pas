...
begin
  if Month = '' then ed_PayMonth.Text := formatDateTime('yyyy년 mm월',now);
  LoadEmployeeInfo;
...
procedure TfmLecturePayment.SetMonth(const Value: string);
begin
  FMonth := Value;
  if Value = '' then Exit;

  ed_PayMonth.Text := copy(Value,1,4) + '년 ' + copy(Value,5,2) + '월';
  MonthCalendar1.Date := strtoDate(copy(Value,1,4) + '-' + copy(Value,5,2) + '-' + formatDateTime('dd',now));
end;
