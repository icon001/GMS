...
type
  ...
  private
    ...
    function SearchFoodGubun(aCode:string):Boolean;
  public
    { Public declarations }
  end;
...
begin
  ...
  begin
    stFoodGubun := dmDBFunction.GetNextTB_FOODGROUP_ID;
    dmDBInsert.InsertIntoTB_FOODGROUP_Value(stFoodGubun,ed_FoodGubunName.text);
  end else
  begin
    dmDBUpdate.UpdateTB_FOODGROUP_Field_StringValue(stFoodGubun,'FG_NAME',ed_FoodGubunName.text);
  end;
...
function TfmFoodGubun.SearchFoodGubun(aCode: string): Boolean;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
  nRow : integer;
begin
  GridInit(sg_List,1,1);

  stSql := dmDBSelect.SelectTB_FOODGROUP_Name('');
  ...
  Try
    ...
    begin
      ...
      begin
        ...
        with sg_List do
        begin
          Cells[0,nRow] := FindField('FG_NAME').AsString;
          Cells[1,nRow] := FindField('FG_CODE').AsString;
...
begin
  ...
  with sg_List do
  begin
    ed_FoodGubunName.Text := cells[0,Row];
