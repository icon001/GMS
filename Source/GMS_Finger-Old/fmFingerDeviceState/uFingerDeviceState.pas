...
begin
  stFingerSeq := tv_Code.Items.Item[tv_List.Selected.AbsoluteIndex].Text;
  nIndex := SupremaFingerList.IndexOf(FillZeroNumber(strtoint(stFingerSeq),G_nNodeCodeLength));
  if nIndex > -1 then
  begin
    TSupremaServer(SupremaFingerList.Objects[nIndex]).FingerUserAllDelete;
  end else
  begin
