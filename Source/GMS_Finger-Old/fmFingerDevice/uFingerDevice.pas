...
begin
  ...
  if Not dmDBInsert.InsertIntoTB_FINGERDEVICE_Value(stFingerID,ed_Ip.Text,ed_Name.Text,ed_ReaderPort.Text,'2',stCPType,stNodeNo,stEcuID,stExtendID,stDoorNo) then
  begin
...
begin
  ...
  if Not dmDBUpdate.UpdateTB_FINGERDEVICE_Value(stFingerSeq,ed_Ip.Text,ed_Name.Text,ed_ReaderPort.Text,'2',stCPType,stNodeNo,stEcuID,stExtendID,stDoorNo) then
  begin
    stMessage := stringReplace(dmFormMessage.GetMessage('DBSAVEERROR'),'$WORK',btn_AddSave.Caption,[rfReplaceAll]);
    Application.MessageBox(PChar(stMessage),'Error',MB_OK);
    Exit;
