...
begin
  ...
  ed_DSState.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCurrentDSState,stEcuID,ed_DoorID.Text);
  ed_CardMode.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCurrentCARDMode,stEcuID,ed_DoorID.Text);
  ed_ManagerMode.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCurrentManagerMode,stEcuID,ed_DoorID.Text);
  ed_DoorRcvState.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCurrentRcvState,stEcuID,ed_DoorID.Text);
  ed_DeviceDoorRcvState.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDInformation(con_DeviceDoorStateRCV,stEcuID);
  ed_DeviceDoorCount.Text := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDInformation(con_DeviceCmdDoorCount,stEcuID);
end;
