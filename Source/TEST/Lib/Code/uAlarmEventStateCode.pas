...
type
  ...
  public
    { Public declarations }
    procedure SetAlarmEventPCCode(aPCcode,aName,aModeChange,aView,aSound,aColor:string);
    procedure SetAlarmEventModeChangeCode(aOperCode,aAlarmEventCode,aAlarmChangeCode:string);
    Function GetAlarmEventPCCodeInfo(aAlarmStateCode,aSubCLass,aMode,aLoop,aZoneState,aOperator:string;var aAlarmStateCodeName,aAlarmStatePCCode,aAlarmStatePCCodeName,aAlarmStateModeChange,aAlarmView,aAlarmSound,aAlarmColor:string):Boolean;
  published
  ...
  private
    ...
    FPCArmModeChange: String;
  public
  published
    property PCArmModeChange : String read FPCArmModeChange Write FPCArmModeChange;
...
function TAlarmEventStateCode.GetAlarmEventPCCodeInfo(aAlarmStateCode,
  ...
  aAlarmStatePCCode, aAlarmStatePCCodeName,aAlarmStateModeChange, aAlarmView, aAlarmSound,
...
begin
  ...
  aAlarmStatePCCode := UpperCase(stPCCode);
  nIndex := AlarmPCCodeList.IndexOf(stPCCode);
  if nIndex < 0 then Exit;
  aAlarmStatePCCodeName := TAlarmEventStatePCCode(AlarmPCCodeList.Objects[nIndex]).PCAlarmEventCodeName;
  aAlarmStateModeChange := TAlarmEventStatePCCode(AlarmPCCodeList.Objects[nIndex]).PCArmModeChange;
  aAlarmView := TAlarmEventStatePCCode(AlarmPCCodeList.Objects[nIndex]).PCAlarmEventView;
  aAlarmSound := TAlarmEventStatePCCode(AlarmPCCodeList.Objects[nIndex]).PCAlarmEventSOUND;
  aAlarmColor := TAlarmEventStatePCCode(AlarmPCCodeList.Objects[nIndex]).PCAlarmEventCOLOR;
  result := True;
end;
...
procedure TAlarmEventStateCode.SetAlarmEventPCCode(aPCcode, aName,aModeChange, aView,
...
begin
  ...
  oAlarmEventStatePCCode.PCArmModeChange := aModeChange;
