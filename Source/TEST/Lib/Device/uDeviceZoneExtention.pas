...
begin
  ...
  begin
    OnZONEEXTENDTYPEChange(Self,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,inttostr(EXTENDID),'0',FZONEEXTENDTYPE);
  end;
end;
...
begin
  ...
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'EXTEND',TDevice(Device).EcuID,inttostr(EXTENDID),'0',con_ExtentionCmdZoneInfoSetting,FZONEINFORCV);
  end;
