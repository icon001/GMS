...
uses
  ...
  uThreadSocket,uDevice,uDeviceVariable;
...
type
  ...
  public
    ...
    procedure ReceiveReaderTelNumberSetting(aECUID,aDeviceVer,aMsgNo,aRealData:string);
...
begin
  ...
  SendPacket('Q',ClientSocket(Device.DeviceNode).GetSendMsgNo,ClientSocket(Device.DeviceNode).GetDeviceVer, stData,0);
  if aDelay then
...
begin
  ...
  stData := 'CV' +                             //MSG Code
            FillZeroNumber(ReaderNo, 2);               //리더번호
  SendPacket('R',ClientSocket(Device.DeviceNode).GetSendMsgNo,ClientSocket(Device.DeviceNode).GetDeviceVer, stData,0);
  if aDelay then
...
procedure TDeviceReader.ReceiveReaderTelNumberSetting(aECUID, aDeviceVer,
  aMsgNo, aRealData: string);
var
  nCmd : integer;
  chTelNo : char;
...
begin
  ...
  stTelNumber := Trim(aRealData);

  case chTelNo of
    '0' : begin
      if (TelNumber0Rcv = 'R') or (TelNumber0Rcv = 'S') then TelNumber0Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber0] := True;
      TelNumber0 := stTelNumber;
    end;
    '1' : begin
      if (TelNumber1Rcv = 'R') or (TelNumber1Rcv = 'S') then TelNumber1Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber1] := True;
      TelNumber1 := stTelNumber;
    end;
    '2' : begin
      if (TelNumber2Rcv = 'R') or (TelNumber2Rcv = 'S') then TelNumber2Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber2] := True;
      TelNumber2 := stTelNumber;
    end;
    '3' : begin
      if (TelNumber3Rcv = 'R') or (TelNumber3Rcv = 'S') then TelNumber3Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber3] := True;
      TelNumber3 := stTelNumber;
    end;
    '4' : begin
      if (TelNumber4Rcv = 'R') or (TelNumber4Rcv = 'S') then TelNumber4Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber4] := True;
      TelNumber4 := stTelNumber;
    end;
    '5' : begin
      if (TelNumber5Rcv = 'R') or (TelNumber5Rcv = 'S') then TelNumber5Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber5] := True;
      TelNumber5 := stTelNumber;
    end;
    '6' : begin
      if (TelNumber6Rcv = 'R') or (TelNumber6Rcv = 'S') then TelNumber6Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber6] := True;
      TelNumber0 := stTelNumber;
    end;
    '7' : begin
      if (TelNumber7Rcv = 'R') or (TelNumber7Rcv = 'S') then TelNumber7Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber7] := True;
      TelNumber7 := stTelNumber;
    end;
    '8' : begin
      if (TelNumber8Rcv = 'R') or (TelNumber8Rcv = 'S') then TelNumber8Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber8] := True;
      TelNumber8 := stTelNumber;
    end;
    '9' : begin
      if (TelNumber9Rcv = 'R') or (TelNumber9Rcv = 'S') then TelNumber9Rcv := 'Y';
      L_bReaderResponse[con_CardReaderCmdTelNumber9] := True;
      TelNumber9 := stTelNumber;
    end;
...
begin
  ...
  SendPacket('I',ClientSocket(Device.DeviceNode).GetSendMsgNo,ClientSocket(Device.DeviceNode).GetDeviceVer, stData,0);
  if aDelay then
    result := ReaderResponseCheck(con_CardReaderCmdSettingInfo,con_DelayTime);
...
begin
  ...
  stData := 'TN' +                                      //COMMAND
    FillZeroNumber(ReaderNo, 2) +                       //리더 번호
    FillZeroNumber(aNo, 2) +                            //등록번지
    FillCharString(aTelNumber,' ',20);                  //전화번호
...
begin
  ...
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdArmAreaNo,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),inttostr(Value));
  end;
end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdBuildingPositionCode,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdDoorNo,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdDoorPosition,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),Value);
  end;
end;
...
begin
  ...
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdCallTime,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdSettingInfo,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdType,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),inttostr(Value));
  end;
end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdUse,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),stValue);
  end;
...
begin
  ...
  if Assigned(FOnCardReaderPacketChange) then
  begin
    OnCardReaderPacketChange(Self,con_CardReaderCmdVERSION,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ReaderNo),Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdVERSION,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber0,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber1,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber2,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber3,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber4,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber5,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber6,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber7,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber8,Value);
  end;
end;
...
begin
  ...
  if Assigned(FOnReceiveTypeChage) then
  begin
    OnReceiveTypeChage(Self,ClientSocket(Device.DeviceNode).NO,'READER',TDevice(Device).EcuID,'0',inttostr(ReaderNo),con_CardReaderCmdTelNumber9,Value);
  end;
end;
