...
uses
  ...
  uThreadSocket,uDeviceVariable,ADODB,ActiveX;
...
type
  ...
  private
    ...
    procedure SetDeviceNode(const Value: ClientSocket);
  ...
  published
    ...
    Property DeviceNode: ClientSocket Read FDeviceNode write SetDeviceNode;
...
begin
  ...
  if CARDTYPE = '' then Exit; //CardType 확인 하지 못한 경우는 빠져 나가자...
  if L_bCardSendCompleteClear then Exit; //카드 정리중이면 빠져 나가자.
  if DeviceCode = '' then Exit;   //DeviceCode = '' 이면 카드 다운로드 하지 말자.




  Try
    nCardDownLoadDelay := 5000; //5초 동안 수신 대기 하자.
    ...
    begin
      ...
      begin
        if TDeviceCard(DeviceCardList.Objects[i]).RCVACK = 'S' then TDeviceCard(DeviceCardList.Objects[i]).RCVACK := 'R';  //송신 중이면 재송신 하자.
...
begin
  ...
  Try
    ...
    begin
      ...
      begin
        OnDeviceCardAccessEvent(Self,inttostr(ClientSocket(DeviceNode).NO),EcuID,stDoorNo,stReaderNo,stInOut,stTime,
      ...
      begin
        Try
          ...
          stTempNodeNo := FillZeroNumber(ClientSocket(DeviceNode).NO,G_nNodeCodeLength);                                                                                                              //yyyymmddhhnnss                                                                                           //16자리
  ...
  Try
    Try
      ...
      begin
        ...
        begin
          OnEmployeePermit(Self,ClientSocket(DeviceNode).NO,EcuID);
        end;
        if Assigned(FOnCardPermit) then
        begin
          OnCardPermit(Self,ClientSocket(DeviceNode).NO,EcuID);
        end;
      end;
...
begin
  Try
    ...
    begin
      ...
      ClientSocket(DeviceNode).KTTCDMADATARCV := 'Y' ;
      Exit;
    end;
...
begin
  Try
    ...
    begin
      ...
      ClientSocket(DeviceNode).KTTCDMAUSERCV := 'Y';
      Exit;
    end;
...
begin
  Try
    ...
    begin
      ...
      ClientSocket(DeviceNode).KTTREMOTERINGCOUNTRCV := 'Y';
      Exit;
    end;
...
begin
  ...
  Try
    ...
    stSql := stSql + ' AND a.ND_NODENO = ' + inttostr(ClientSocket(DeviceNode).NO) + '';
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).ARMEXTENTIONMAINFROMLOCALRCV = 'R') OR (ClientSocket(DeviceNode).ARMEXTENTIONMAINFROMLOCALRCV = 'S') then ClientSocket(DeviceNode).ARMEXTENTIONMAINFROMLOCALRCV := 'Y'; //수신상태가 송신 중이거나 수신 중일때 수신 완료 처리
    Delete(aRealData,1,4);
    ClientSocket(DeviceNode).ARMEXTENTIONMAINFROMLOCAL := aRealData;
  Except
...
begin
  Try
    L_bDeviceResponse[con_DeviceCmdARMEXTENTIONMAINFROMLOCAL] := True;
    if (ClientSocket(DeviceNode).ARMEXTENTIONMAINTOLOCALRCV = 'R') OR (ClientSocket(DeviceNode).ARMEXTENTIONMAINTOLOCALRCV = 'S') then ClientSocket(DeviceNode).ARMEXTENTIONMAINTOLOCALRCV := 'Y'; //수신상태가 송신 중이거나 수신 중일때 수신 완료 처리
    Delete(aRealData,1,4);
    ClientSocket(DeviceNode).ARMEXTENTIONMAINTOLOCAL := aRealData;
  Except
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).DVRDATARCV = 'R') or (ClientSocket(DeviceNode).DVRDATARCV = 'S') then
      ClientSocket(DeviceNode).DVRDATARCV := 'Y';
    Delete(aRealData,1,4);
    ClientSocket(DeviceNode).DVRIP := FindCharCopy(aRealData, 0, ' ');
    ClientSocket(DeviceNode).DVRPORT := FindCharCopy(aRealData, 1, ' ');
  Except
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).DVRUSERCV = 'R') or (ClientSocket(DeviceNode).DVRUSERCV = 'S') then
        ClientSocket(DeviceNode).DVRUSERCV := 'Y';
    Delete(aRealData,1,4);
    ClientSocket(DeviceNode).DVRUSE := aRealData[1];
  Except
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).ECUUSERCV = 'R') or (ClientSocket(DeviceNode).ECUUSERCV = 'S') then
        ClientSocket(DeviceNode).ECUUSERCV := 'Y';
    Delete(aRealData,1,4); //command 부분 삭제 처리
    if Assigned(FOnDeviceReceiveDataEvent) then
    begin
      OnDeviceReceiveDataEvent(Self,ClientSocket(DeviceNode).No,'DEVICE',aEcuID,'0','0',con_DeviceCmdUseState,aRealData);
    end;
  Except
...
begin
  Try
    L_bDeviceResponse[con_DeviceCmdKttCDMADATA] := True;
    if (ClientSocket(DeviceNode).KTTCDMADATARCV = 'R') or (ClientSocket(DeviceNode).KTTCDMADATARCV = 'S') then
        ClientSocket(DeviceNode).KTTCDMADATARCV := 'Y';
    if aEcuID <> '00' then Exit;
    Delete(aRealData,1,4);
    stTemp  := FindCharCopy(aRealData, 0, '}');
    ClientSocket(DeviceNode).KTTCDMAMIN := stTemp;
    stTemp  := FindCharCopy(aRealData, 1, '}');
    ClientSocket(DeviceNode).KTTCDMAMUX := stTemp;
    stTemp  := FindCharCopy(aRealData, 2, '}');
    ClientSocket(DeviceNode).KTTCDMAIP := stTemp;
    stTemp  := FindCharCopy(aRealData, 3, '}');
    ClientSocket(DeviceNode).KTTCDMAPORT := stTemp;
    stTemp  := FindCharCopy(aRealData, 4, '}');
    ClientSocket(DeviceNode).KTTCDMACHECKTIME := stTemp;
    stTemp  := FindCharCopy(aRealData, 5, '}');
    ClientSocket(DeviceNode).KTTCDMARSSI := stTemp;
  Except
...
begin
  Try
    L_bDeviceResponse[con_DeviceCmdKttCdmaUse] := True;
    if (ClientSocket(DeviceNode).KTTCDMAUSERCV = 'R') or (ClientSocket(DeviceNode).KTTCDMAUSERCV = 'S') then
        ClientSocket(DeviceNode).KTTCDMAUSERCV := 'Y';
    Delete(aRealData,1,4);
    ClientSocket(DeviceNode).KTTCDMAUSE := aRealData[1];
  Except
...
begin
  Try
    L_bDeviceResponse[con_DeviceCmdKTTREMOTEARMRINGCOUNT] := True;
    if (ClientSocket(DeviceNode).KTTREMOTERINGCOUNTRCV = 'R') or (ClientSocket(DeviceNode).KTTREMOTERINGCOUNTRCV = 'S') then ClientSocket(DeviceNode).KTTREMOTERINGCOUNTRCV := 'Y';
    Delete(aRealData,1,4);
    stArmCount := copy(aRealData,1,2);
    stDisArmCount := copy(aRealData,3,2);
    if isDigit(stArmCount) then
      ClientSocket(DeviceNode).KTTREMOTEARMRINGCOUNT := strtoint(stArmCount);
    if isDigit(stDisArmCount) then
      ClientSocket(DeviceNode).KTTREMOTEDISARMRINGCOUNT := strtoint(stDisArmCount);
  Except
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).KTTSYSTEMIDRCV = 'R') or (ClientSocket(DeviceNode).KTTSYSTEMIDRCV = 'S') then ClientSocket(DeviceNode).KTTSYSTEMIDRCV := 'Y';
    ...
    ClientSocket(DeviceNode).KTTSYSTEMID := stTemp;
  Except
...
begin
  Try
    ...
    if (ClientSocket(DeviceNode).KTTTELNUMBERRCV = 'R') or (ClientSocket(DeviceNode).KTTTELNUMBERRCV = 'S') then ClientSocket(DeviceNode).KTTTELNUMBERRCV := 'Y';
    Delete(aRealData,1,4);
    stTemp := Copy(aRealData,1,2);
    Delete(aRealData,1,2);
    if stTemp = '00' then ClientSocket(DeviceNode).KTTTELNUMBER1 := aRealData
    else if stTemp = '01' then ClientSocket(DeviceNode).KTTTELNUMBER2 := aRealData;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDeviceReceiveDataEvent) then
    begin
      OnDeviceReceiveDataEvent(Self,ClientSocket(DeviceNode).No,'DEVICE',aEcuID,'0','0',con_DeviceCmdNetworkState,aRealData);
    end;
  Except
  ...
  Try
    ...
    case msgcode of
      'a','b','g': begin //출입문 등록 응답
        ...
        stTempNodeNo := FillZeroNumber(ClientSocket(DeviceNode).NO,G_nNodeCodeLength);
...
begin
  Try
    ...
    if nIndex > -1 then
    begin
      if stAlarmStateCode <>  G_stLongTimeStateCode then    //장시간 열림시 현재 모드를 변경하면 안됨
    ...
    begin
      OnCurrentAlarmEvent(Self,inttostr(ClientSocket(DeviceNode).NO),aECUID,aCmd,aMsgNo,stTime,stSubCLass,stSubAddr,stArmArea,stMode,stAlarmStateCode,stLoop,stZoneState,stOperator,stAlarmStateCodeName,stAlarmStatePCCode,stAlarmStatePCCodeName,stAlarmStateModeChange,stAlarmView,stAlarmSound,stAlarmColor);
    end;
...
begin
  Try
    ...
    begin
      L_bDeviceResponse[con_DeviceCmdARMEXTENTIONMAINFROMLOCAL] := True;
      ClientSocket(DeviceNode).ARMEXTENTIONMAINFROMLOCALRCV := 'Y';
    end else if stCmd = 'EX04' then
    begin
      L_bDeviceResponse[con_DeviceCmdARMEXTENTIONMAINTOLOCAL] := True;
      ClientSocket(DeviceNode).ARMEXTENTIONMAINTOLOCALRCV := 'Y';
    ...
    begin
      L_bDeviceResponse[con_DeviceCmdKTTSystemID] := True;
      ClientSocket(DeviceNode).KTTSYSTEMIDRCV := 'Y';
    ...
    begin
      L_bDeviceResponse[con_DeviceCmdKTTTelNumber1] := True;
      ClientSocket(DeviceNode).KTTTELNUMBERRCV := 'Y';
  ...
  //    end;
    ...
    begin
      ...
      begin
        ...
        begin
          OnAntiPassDataEvent(Self,ClientSocket(DeviceNode).No,aEcuID,stCmd,'',aRealData);
        end;
      end else
      begin
        for i := 0 to AntiGroupList.Count - 1 do
          OnAntiPassDataEvent(Self,ClientSocket(DeviceNode).NO,EcuID,stCmd,AntiGroupList.Strings[i],aRealData);
      end;
...
begin
  Try
    ...
    begin
      OnDevicePacketChange(Self,con_DeviceCmdArmAreaUse,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdArmInDelay,ClientSocket(DeviceNode).NO,EcuID,inttostr(Value));
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdArmOutDelay,ClientSocket(DeviceNode).NO,EcuID,inttostr(Value));
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdArmRelay,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdCardReaderType,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if EcuID = '00' then
    begin
      if Value = KT811 then ClientSocket(DeviceNode).DVRSkill := True
      else ClientSocket(DeviceNode).DVRSkill := False;
    end;
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdArmAreaUse,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdArmRelay,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdCardReaderNetwork,FDEVICECARDREADERNETWORKRcv);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdCardReaderType,FDEVICECARDREADERTYPERCV);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdDeviceCode,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdDeviceCode,Value);
    end;
  Except
...
begin
  Try
    ...
    begin
      ...
      ClientSocket(DeviceNode).NodeConnected := csConnected;
      ClientSocket(DeviceNode).DoorArmAreaStateRCV := False;  //확장기
      ClientSocket(DeviceNode).ReciveStateChange := True; //컨트롤러 접속하면 기기정보 송수신 상태 체크 하자
    end else
    begin
    ...
    begin
      if AntiGroupList.Count < 1 then
      begin
        OnDeviceConnected(Self,ClientSocket(DeviceNode).WinSocket,ClientSocket(DeviceNode).NO,EcuID,Value,'');
      end else
      begin
        for i := 0 to AntiGroupList.Count - 1 do
          OnDeviceConnected(Self,ClientSocket(DeviceNode).WinSocket,ClientSocket(DeviceNode).NO,EcuID,Value,AntiGroupList.Strings[i]);
      end;
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdExtentionUse,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdDeviceDoor2RelayType,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdDeviceDoorArmArea,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdEMZONELAMP,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdExtentionNetwork,FDEVICEExtentionNetworkRCV);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdJaeJungDelayUse,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdJAVARAARMCLOSE,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdJAVARAAUTOCLOSE,Value);
    end;
  Except
...
procedure TDevice.SetDeviceNode(const Value: ClientSocket);
var
  i,j : integer;
  oDoor : TDeviceDoor;
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdTimeCodeSend,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdTimeCodeUse,Value);
    end;
  Except
    ...
    case Value[1] of
      KTT811 : begin
        ...
        begin
          ...
          if EcuID = '00' then
          begin
            ClientSocket(DeviceNode).DVRSKILL := True;
          end;
        end else
        begin
          ...
          if EcuID = '00' then
          begin
            if CardType <> '' then ClientSocket(DeviceNode).DVRSKILL := False;
          end;
        end;
        FTPSkill := True;
        ...
        if EcuID = '00' then
        begin
          ClientSocket(DeviceNode).KTTCDMASKILL := False;
          ClientSocket(DeviceNode).ARMEXTENTIONSKILL:= False;
        end;
      ...
      KTT812 : begin
        ...
        if EcuID = '00' then
        begin
          ClientSocket(DeviceNode).KTTCDMASKILL := True;
          ClientSocket(DeviceNode).DVRSKILL := False;
          ClientSocket(DeviceNode).ARMEXTENTIONSKILL:= True;
        end;
        TDeviceDoor(DeviceDoorList.Objects[0]).DoorUse := True;  //1번 출입문 사용 가능
      end;
      ICU100 : begin
...
begin
  Try
    ...
    begin
      OnDeviceUsed(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdDeviceVersion,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdVER,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdSystemInformation,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdLAMPONTIME,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnReceiveTypeChage) then
    begin
      OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdSIRENONTIME,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdDeviceDoor2RelayType,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdEMZONELAMP,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdEMZONESIREN,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    begin
      ...
      if Assigned(FOnReceiveTypeChage) then
      begin
        OnReceiveTypeChage(Self,ClientSocket(DeviceNode).NO,'DEVICE',EcuID,'0','0',con_DeviceCmdFTPCardDownLoadEnd,'Y');
      end;
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJaeJungDelayUse,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARAARMCLOSE,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARAAUTOCLOSE,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARADISARMOPEN,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARASERVERARMCLOSE,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARASERVERDISARMOPEN,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdJAVARATYPEUSE,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdLAMPONTIME,ClientSocket(DeviceNode).NO,EcuID,inttostr(Value));
    end;
  Except
...
begin
  Try
    ...
    if Value then
    begin
       ClientSocket(DeviceNode).ReciveStateChange := True;
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdSIRENONTIME,ClientSocket(DeviceNode).NO,EcuID,inttostr(Value));
    end;
  Except
...
begin
  ...
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdTimeCodeSkill,ClientSocket(DeviceNode).NO,EcuID,stValue);
    end;
  Except
...
begin
  Try
    ...
    if Assigned(FOnDevicePacketChange) then
    begin
      OnDevicePacketChange(Self,con_DeviceCmdWATCHACPOWER,ClientSocket(DeviceNode).NO,EcuID,Value);
    end;
  Except
