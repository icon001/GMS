...
uses
  ...
  uDevice,uDeviceVariable,uThreadSocket;
...
begin
  ...
  begin
    ...
    begin
      OnArmAreaPacket(Self,con_ArmAreaCmdModeChange,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ArmAreaNo),Value);
    end;
  end;
end;
...
begin
  ...
  if Assigned(FOnArmAreaPacket) then
  begin
    OnArmAreaPacket(Self,con_ArmAreaCmdUse,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ArmAreaNo),stValue);
  end;
...
begin
  ...
  if Assigned(FOnArmAreaPacket) then
  begin
    OnArmAreaPacket(Self,con_ArmAreaCmdView,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,'0',inttostr(ArmAreaNo),Value);
  end;
end;
