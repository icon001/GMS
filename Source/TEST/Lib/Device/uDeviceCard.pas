...
uses
  ...
  uDeviceVariable,uThreadSocket,uDevice;
...
begin
  ...
  begin
    ClientSocket(Device.DeviceNode).CardDownLoadStart := True;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      OnCardPermitChange(Self,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,CARDNO,stPermit,stDoorPermit,stArmPermit,L_stTimeCodeUse,L_stTimeGroup,stTimeCode,L_stWeekCode);
    end;
...
begin
  ...
  begin
    OnCardRcvAckChange(Self,ClientSocket(Device.DeviceNode).NO,TDevice(Device).EcuID,CARDNO,Value,CARDPERMIT);
  end;
end;
