...
begin
  ...
  stNodeNo := inttostr(ClientSocket(NodeList.Objects[aIndex]).No);
  stEcuID := '00';
  //    'N' : begin  //네트웍 상태
  stSendData := GetNodeDeviceNetworkState(stNodeNo);
  SendPacket('S',SCECU,'N',stNodeNo,stEcuID,stSendData);
  nCount := strtoint(ClientSocket(NodeList.Objects[aIndex]).GetNodeIndexInformation(con_NodeCmdDeviceCount));
  for i := 0 to nCount - 1 do
  begin
    stEcuID := ClientSocket(NodeList.Objects[aIndex]).GetDeviceIndexInformation(con_DeviceCmdDeviceID,i);
  ...
  Try
    ...
    case aRealData[3] of
      'M' : begin  //방범구역 모드 변경
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).SendPacket(stEcuID,'R',ClientSocket(NodeList.Objects[nIndex]).GetSendMsgNo,ClientSocket(NodeList.Objects[nIndex]).GetDeviceVer,stData,1);
        end else
        begin
  ...
  Try
    ...
    case aRealData[3] of
      's' : begin  //출입문 상태
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).ControlNodeToDeviceArmAreaStateCheck(stEcuID,stArmAreaNo);
        end else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
          if nIndex > -1 then
  ...
  Try
    ...
    case aRealData[3] of
      'O' : begin  //출입문 열기
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).SendPacket(stEcuID,'c',ClientSocket(NodeList.Objects[nIndex]).GetSendMsgNo,ClientSocket(NodeList.Objects[nIndex]).GetDeviceVer,stData,1);
        end else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
          if nIndex > -1 then
      ...
      'S' : begin  //출입문 스케줄 사용유무 변경
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).SetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCmdSCHEDULEUSE,stEcuID,stDoorNo,stTemp);
          ClientSocket(NodeList.Objects[nIndex]).SetDeviceRcvIDSubInformation(con_DeviceTypeDoor,con_DoorCmdSettingInfo,stEcuID,stDoorNo,'U');
        end
        else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
      ...
      'T' : begin  //출입문 타임코드 사용유무 변경
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).SetDeviceIDSubInformation(con_DeviceTypeDoor,con_DoorCmdTimeCodeUse,stEcuID,stDoorNo,stTemp);
          ClientSocket(NodeList.Objects[nIndex]).SetDeviceRcvIDInformation(con_DeviceCmdTimeCodeUse,stEcuID,'U');
        end
        else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
  ...
  Try
    ...
    case aRealData[3] of
      'C' : begin  //카드 재전송
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).ControlNodeToDeviceCardResend(stEcuID);
        end else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
          if nIndex > -1 then
      ...
      'R' : begin  //컨트롤러 리셋
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).ControlNodeToDeviceReset(stEcuID);
        end else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
          if nIndex > -1 then
      ...
      'F' : begin  //화재 복구
        ...
        begin
          ...
          begin
            ...
            begin
              ClientSocket(NodeList.Objects[i]).FireRecovery;
            end;
          end;
        ...
        begin
          ...
          if nIndex > -1 then
          begin
            ClientSocket(NodeList.Objects[nIndex]).FireRecovery;
          end else
          begin
            nIndex :=  ServerNodeList.IndexOf(stNodeNo);
  ...
  Try
    ...
    case aRealData[3] of
      'T' : begin  //시간동기화
        ...
        if nIndex > -1 then
        begin
          ClientSocket(NodeList.Objects[nIndex]).ControlNodeToDeviceTimeSync;
        end else
        begin
          nIndex := ServerNodeList.IndexOf(stNodeNo);
          if nIndex > -1 then
...
begin
  result := '';
  if aIndex > NodeList.Count - 1 then Exit;
  stNodeData := FillZeroNumber(ClientSocket(NodeList.Objects[aIndex]).No,G_nNodeCodeLength);
  stDoorData := FillZeroNumber(0,8); //8bit 처리
  if ClientSocket(NodeList.Objects[aIndex]).NodeConnected = csConnected then
  begin
    stDoorData[1] := '1'; //노드 연결 되어 있다.
    if ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentFireEvent,'00','1') = '1' then
    begin
      stDoorData[8] := '1';
      stTemp := ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentDSState,'00','1');
      if stTemp <> '*' then
      begin
    ...
    if ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentCARDMode,'00','1') = '1' then  stDoorData[3] := '1';
    stTemp := ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentManagerMode,'00','1');
...
begin
  Try
    ...
    if nIndex > -1 then
    begin
      //if Not ClientSocket(NodeList.Objects[nIndex]).start then Exit;
      stTemp := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIDInformation(con_DeviceCmdArmAreaState,aEcuID);
...
begin
  Try
    ...
    begin
      ...
      stTemp := ClientSocket(NodeList.Objects[nIndex]).GetNodeIndexInformation(con_NodeCmdDeviceCount);
      ...
      begin
        for i := 0 to nCount - 1 do
        begin
          stTemp := ClientSocket(NodeList.Objects[nIndex]).GetDeviceIndexInformation(con_DeviceCmdDeviceConnected,i);
...
begin
  ...
  begin
    ...
    begin
      ...
      if ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentFireEvent,'00',inttostr(i+1)) = '1' then
      begin
        stDoorData[i*8 + 8] := '1';
        stTemp := ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentDSState,'00',inttostr(i+1));
        if stTemp <> '*' then
        begin
      ...
      if ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentCARDMode,'00',inttostr(i+1)) = '1' then  stDoorData[i*8 + 3] := '1';
      stTemp := ClientSocket(NodeList.Objects[aIndex]).GetDeviceIDDoorInformation(con_DoorCurrentManagerMode,'00',inttostr(i+1));
...
begin
  ...
  if stSendData <> '' then
     SendPacket('S',SCDOOR,'O',inttostr((NodeList.Count) mod G_nMonitoringOnlyMainCount),'00',stSendData);
end;
