...
procedure LoadAdminGRADECode(aStringList:TStringList;cmb_Box:TComboBox;aAll:Boolean=True;aAllName:string='전체');
...
procedure LoadMenuGroupCode(aProgramGroupCode:string;aStringList:TStringList;cmb_Box:TComboBox;aAll:Boolean=True;aAllName:string='전체');
...
begin
  ...
  Try
    ...
    begin
      ...
      While Not Eof do
      begin
        cmb_Box.Items.Add(FindField('GR_GARDENAME').AsString);
        aStringList.Add(FindField('GR_GRADECODE').AsString);
        Next;
      end;
      if RecordCount = 1 then
      begin
...
begin
  ...
  stSql := stSql + ' Where GROUP_CODE = ''' + G_stGroupCode + ''' ';
  if aProgramGroupCode <> '' then stSql := stSql + ' AND GUBUN = ''' + aProgramGroupCode + ''' ';
  ...
  Try
    ...
    begin
      ...
      While Not Eof do
      begin
        cmb_Box.Items.Add(FindField('PR_GROUPCODENAME').AsString);
        aStringList.Add(FindField('PR_GROUPCODE').AsString);
        Next;
      end;
      if RecordCount = 1 then
      begin
...
begin
  ...
  cmb_Box.Items.Add('출입통제관제시스템');
  aStringList.Add('MONITORING');
  cmb_Box.Items.Add('근태시스템');
  aStringList.Add('ATTEND');
  cmb_Box.Items.Add('식수시스템');
  aStringList.Add('FOOD');
