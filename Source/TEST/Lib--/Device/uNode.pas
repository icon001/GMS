...
uses
  ...
  Vcl.Graphics,u_c_byte_buffer, IdBaseComponent, IdComponent, IdTCPConnection,
  IdTCPClient;
...
type
  TTCPReadEvent = procedure (Sender : TObject; Socket: TIdTCPConnection; Data: TMemoryStream) of object;
  ...
  public
    ...
    procedure TCPClientRead(Sender : TObject; Socket: TIdTCPConnection;Data: TMemoryStream);
...
//  l_c_reception_buffer:= c_byte_buffer.create_byte_buffer('reception_buffer', k_buffer_max);
  IndySocket := TIdTcpClient.Create(self);
  IndySocket.OnConnected := TCPClientConnected;
  IndySocket.OnStatus := TCPClientStatus;
...
begin
  ...
  begin
    try
      IndySocket.Host := Trim(NODEIP);
      IndySocket.Port := NodePort;
      IndySocket.Connect;
...
procedure TNode.TCPClientConnected(Sender: TObject);
begin
  NodeConnected := csConnected;
end;

procedure TNode.TCPClientRead(Sender: TObject; Socket: TIdTCPConnection;
  Data: TMemoryStream);
var
  i : Integer;
...
procedure TNode.TCPClientStatus(ASender: TObject; const AStatus: TIdStatus;
  const AStatusText: string);
begin
  case AStatus of
    hsResolving     : ;
