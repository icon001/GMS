...
uses
  ...
  AdvToolBarStylers, AdvOfficeTabSet, AdvOfficeTabSetStylers;
...
begin
  ...
  begin
    ...
    begin
      TAdvTabSheet(TabList.Objects[i]).Free;
    end;
    TabList.Clear;
  end;
  ...
  begin
    oTabSheet := TAdvTabSheet.Create(PageBuilding);
    with oTabSheet do
    begin
      Caption := '출입';
      Parent := PageBuilding;
  ...
  begin
    ...
    begin
      ...
      with oTabSheet do
      begin
        Caption := TMonitoringBuildingState(G_MonitoringBuildingList.Objects[i]).BuildingName;
        Parent := PageBuilding;
        Visible := True;
      end;
      TabList.AddObject(stBuildingCode,oTabSheet);
      oDoorMonitorForm := TfmBuildingCodeDoorState.Create(nil);
