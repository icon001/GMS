...
begin
  ...
  if Not dmDBInsert.InsertIntoTB_FINGERDEVICE_Value(stFingerID,ed_Ip.Text,ed_Name.Text,ed_ReaderPort.Text,inttostr(cmb_ReaderType.ItemIndex + 1),stCPType,stNodeNo,stEcuID,stExtendID,stDoorNo,ed_FingerDeviceID.Text) then
  begin
...
begin
  ...
  if Not dmDBUpdate.UpdateTB_FINGERDEVICE_Value(stFingerSeq,ed_Ip.Text,ed_Name.Text,ed_ReaderPort.Text,inttostr(cmb_ReaderType.ItemIndex + 1),stCPType,stNodeNo,stEcuID,stExtendID,stDoorNo,ed_FingerDeviceID.Text) then
  begin
    stMessage := stringReplace(dmFormMessage.GetMessage('DBSAVEERROR'),'$WORK',btn_AddSave.Caption,[rfReplaceAll]);
    Application.MessageBox(PChar(stMessage),'Error',MB_OK);
    Exit;
