...
begin
  ...
  begin
    showmessage(stFingerIP + ':' + '데이터 타입 오류 입니다.');
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if G_nMonitoringType = 1 then   //분기국사인경우
    begin
      ap_AccessDevice.TabVisible := False;
      ap_AccessDoor.TabVisible := False;
    end;
    if G_nMonitoringProtocol = 1 then  //주장치만 사용하는 경우
    begin
      ap_AccessDevice.TabVisible := False;
    end;
...
begin
  ...
  stMessage := stCardNo + '의 지문정보를 삭제 하는 것이 정말 맞습니까?';
  ...
  begin
    ...
    begin
      if Not dmSHFDBRelay.CheckTB_Approvalnum(stFingerID,inttostr(nFingerReaderID)) then dmSHFDBRelay.InsertIntoTb_approvalnum(stFingerID,inttostr(nFingerReaderID),'','S','T');  //이 테이블에 있어야 전송한다.
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AddImageIdx(0,i + 1,8,haLeft,vaCenter); //방범구역이미지
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AddImageIdx(0,i + 1,12,haLeft,vaCenter); //출입문이미지
...
begin
  inherited;
  if G_nMonitoringType = 1 then Exit; //분기국사 타입이면 바로 빠져 나가자.
...
begin
  ...
  if G_nMonitoringProtocol = 1 then  //주장치 전용 -> 바로 방범구역 및 출입문 상태로 넘어 가자.
  begin
    stEcuID := '00';
    LoadDoorGrid(stServerNo,stNodeNo,stEcuID,sg_AccessDoor);
