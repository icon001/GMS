...
begin
  ...
  lb_ArmAreaName.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONDOORNAME');
...
begin
  ...
  end else if cmb_AlarmGroup.ItemIndex = 1 then
  begin
    stSql := stSql + ' AND (a.AE_EVENTSTATECODE = ''NF'' OR a.AE_EVENTSTATECODE = ''NR'' ) ';
  end else if cmb_AlarmGroup.ItemIndex = 2 then
  begin
...
procedure TfmDoorAlarmReport.tv_buildingNameDblClick(Sender: TObject);
begin
  inherited;
