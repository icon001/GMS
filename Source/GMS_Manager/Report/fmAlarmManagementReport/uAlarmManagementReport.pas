...
type
  TfmAlarmManagementReport = class(TfmASubForm)
    Image1: TImage;
...
var
  fmAlarmManagementReport: TfmAlarmManagementReport;
...
function TfmAlarmManagementReport.AccessReportHeaderNameSetting(aList: TAdvStringGrid): Boolean;
var
  i : integer;
begin
...
procedure TfmAlarmManagementReport.btn_ExcelClick(Sender: TObject);
var
...
procedure TfmAlarmManagementReport.btn_OptionCancelClick(Sender: TObject);
begin
  inherited;
  Pan_Option.Visible := False;
end;

procedure TfmAlarmManagementReport.btn_OptionClick(Sender: TObject);
var
  i : integer;
begin
  inherited;
...
procedure TfmAlarmManagementReport.btn_OptionSaveClick(Sender: TObject);
var
  ini_fun : TiniFile;
  i : integer;
begin
  Try
    ini_fun := TiniFile.Create(G_stExeFolder + '\ArmAreaManagementReport.INI');
    with ini_fun do
    begin
...
procedure TfmAlarmManagementReport.btn_PrintClick(Sender: TObject);
begin
  sg_Report.Print;
end;

procedure TfmAlarmManagementReport.btn_SearchClick(Sender: TObject);
begin
  inherited;
  SearchReport;
end;

procedure TfmAlarmManagementReport.ChangeAccessReportIndex(FromIndex, ToIndex: integer);
var
  i : integer;
  nChangData : integer;
begin
...
procedure TfmAlarmManagementReport.eb_BuildingNameClickBtn(Sender: TObject);
begin
  inherited;
...
procedure TfmAlarmManagementReport.ed_AddNameKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key = #13 then
...
procedure TfmAlarmManagementReport.ed_nameKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #13 then
  begin
...
procedure TfmAlarmManagementReport.FontSetting;
begin
...
procedure TfmAlarmManagementReport.FormActivate(Sender: TObject);
begin
  inherited;
  WindowState := wsMaximized;
end;

procedure TfmAlarmManagementReport.FormChangeEvent(aFormName: integer);
begin
end;

procedure TfmAlarmManagementReport.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  WriteIniConfig;

  fmMain.FORMENABLE(con_FormBMOSArmAreaMangementREPORT,'FALSE');

  MemoryFree;
  Action := caFree;
end;

function TfmAlarmManagementReport.FormComboBoxInitialize: Boolean;
begin
...
procedure TfmAlarmManagementReport.FormCreate(Sender: TObject);
begin
...
procedure TfmAlarmManagementReport.FormGradeRefresh;
begin
//    btn_Excel.Enabled := IsUpdateGrade;
//    btn_Print.Enabled := IsInsertGrade;
end;

function TfmAlarmManagementReport.FormGridInitialize: Boolean;
begin
  GridInitialize(sg_Report);
  ReportCellHide(sg_Report);
end;

procedure TfmAlarmManagementReport.FormIDSetting(aID: string);
begin
  L_stMenuID := aID;
end;

function TfmAlarmManagementReport.FormNameSetting: Boolean;
var
  i : integer;
begin
  ...
  lb_List.Caption.Text := dmFormFunction.GetFormName('2','1','BM4_019');
...
procedure TfmAlarmManagementReport.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TfmAlarmManagementReport.FormShow(Sender: TObject);
begin
  fmMain.FORMENABLE(con_FormBMOSArmAreaMangementREPORT,'TRUE');
...
function TfmAlarmManagementReport.MemoryCreate: Boolean;
begin
...
function TfmAlarmManagementReport.MemoryFree: Boolean;
begin
...
function TfmAlarmManagementReport.ReportCellHide(aList: TAdvStringGrid): Boolean;
var
  i : integer;
begin
...
function TfmAlarmManagementReport.ReportConfigRead: Boolean;
var
  ini_fun : TiniFile;
  nShowCount : integer;
  nSize : integer;
...
function TfmAlarmManagementReport.ReportFieldNameSetting: Boolean;
begin
...
function TfmAlarmManagementReport.ReportHeaderNameSetting: Boolean;
begin
...
function TfmAlarmManagementReport.SearchReport: Boolean;
var
  stStartDate : string;
  stStartTime : string;
  stEndDate : string;
...
begin
  ...
  stSql :=  dmDBSelect.SelectTB_SYSTEMLOG_ArmAreaManagementReport(stStartDate, stStartTime,
            stEndDate, stEndTime, stBuildingCode, stNodeNo, stEcuID, stArmAreaNo,1);
...
procedure TfmAlarmManagementReport.sg_reportColumnMoved(Sender: TObject; FromIndex,
  ToIndex: Integer);
begin
  inherited;
...
procedure TfmAlarmManagementReport.tv_buildingNameDblClick(Sender: TObject);
begin
  inherited;
...
function TfmAlarmManagementReport.WriteIniConfig: Boolean;
var
  ini_fun : TiniFile;
  i : integer;
begin
...
initialization
  RegisterClass(TfmAlarmManagementReport);
Finalization
  UnRegisterClass(TfmAlarmManagementReport);
