...
uses
  ...
  AdvAppStyler, AdvEdBtn, Vcl.ImgList, Vcl.ComCtrls;
...
type
  TfmNodeManager = class(TfmASubForm)
    ...
    lb_AddFireGroup: TAdvSmoothLabel;
    ...
    lb_SearchNodeIP: TAdvSmoothLabel;
...
function TfmNodeManager.AddNodeWork: Boolean;
var
  stNodeNo : string;
  stEcuID : string;
  stMessage : string;
...
begin
  ...
  //if cmb_ServerNo.ItemIndex < 0 then cmb_ServerNo.ItemIndex := 0;
  ...
  begin
    ...
    begin
      if j <= nViewDoor then
         dmDBInsert.InsertIntoTB_DOOR_DeviceDoorInit(stNodeNo,stEcuID,inttostr(j),ed_NodeName.Text + '_' + stEcuID + '_' + inttostr(j),'1','1','',ed_AddBuildingCode.Text )
      else
         dmDBInsert.InsertIntoTB_DOOR_DeviceDoorInit(stNodeNo,stEcuID,inttostr(j),ed_NodeName.Text + '_' + stEcuID + '_' + inttostr(j),'0','0','',ed_AddBuildingCode.Text );
    ...
    begin
      if j = 0 then
        dmDBInsert.InsertIntoTB_ARMAREA_DeviceArmAreaInit(stNodeNo,stEcuID,inttostr(j),ed_NodeName.Text + '_' + stEcuID + '_' + inttostr(j),'1','1',ed_AddBuildingCode.Text  )
      else dmDBInsert.InsertIntoTB_ARMAREA_DeviceArmAreaInit(stNodeNo,stEcuID,inttostr(j),ed_NodeName.Text + '_' + stEcuID + '_' + inttostr(j),'0','0',ed_AddBuildingCode.Text  );
      Application.ProcessMessages;
    end;
    ...
    begin
      ...
      begin
        dmDBInsert.InsertIntoTB_ZONE_ZoneInit(stNodeNo,stEcuID,inttostr(j),inttostr(k),ed_NodeName.Text + '_' + stEcuID + '_' + inttostr(j)+ '_' + inttostr(k),ed_AddBuildingCode.Text);
    ...
    begin
      dmDBInsert.InsertIntoTB_READER_ReaderInit(stNodeNo,stEcuID,inttostr(j),ed_AddBuildingCode.Text);
    ...
    begin
      ...
      dmDBInsert.InsertIntoTB_DEVICE_DeviceInit(stNodeNo,stEcuID,inttostr(0),ed_NodeName.Text + '_' + stEcuID ,'1',ed_AddBuildingCode.Text);
      dmDBInsert.InsertIntoTB_DEVICERCV_DeviceInit(stNodeNo,stEcuID,inttostr(0));
      Application.ProcessMessages;
    end;
...
procedure TfmNodeManager.AdvStrinGridSetAllCheck(Sender: TObject;
  bchkState: Boolean);
var
  i : integer;
begin
...
procedure TfmNodeManager.btn_CloseClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TfmNodeManager.btn_DeleteClick(Sender: TObject);
var
  i : integer;
  bChkState : Boolean;
  stMessage : string;
...
procedure TfmNodeManager.btn_ExcelClick(Sender: TObject);
var
...
procedure TfmNodeManager.btn_FireGroupAddClick(Sender: TObject);
var
  nIndex : integer;
begin
  inherited;
...
procedure TfmNodeManager.btn_FireGroupCancelClick(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.btn_FireGroupClick(Sender: TObject);
var
  i : integer;
  bChkState : Boolean;
  stMessage : string;
...
procedure TfmNodeManager.btn_FireGroupSelectClick(Sender: TObject);
var
  i : integer;
  bChkState : Boolean;
...
procedure TfmNodeManager.btn_minimizeClick(Sender: TObject);
begin
  inherited;
  windowState := wsNormal;
end;

procedure TfmNodeManager.btn_SaveClick(Sender: TObject);
var
  stNodeNo : string;
  stEcuID : string;
  stMessage : string;
...
(*  if cmb_ServerNo.ItemIndex > -1 then
    stServerNo := ServerNoList.Strings[cmb_ServerNo.ItemIndex];
  nIndex := PCClientSocketList.IndexOf(inttostr(stServerNo));
  if nIndex > -1 then
  begin
    TdmPCClient(PCClientSocketList.Objects[nIndex]).SendPacket();
  end;
*)
end;

procedure TfmNodeManager.btn_SearchClick(Sender: TObject);
begin
  inherited;
  SearchList('');
end;

procedure TfmNodeManager.btn_UpdateClick(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.CommandArrayCommandsTGRADEREFRESHExecute(
...
procedure TfmNodeManager.CommandArrayCommandsTMENUIDExecute(Command: TCommand;
  Params: TStringList);
begin
  inherited;
...
procedure TfmNodeManager.eb_BuildingNameClickBtn(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.ed_AddBuildingNameClickBtn(Sender: TObject);
begin
  inherited;
  tv_AddbuildingName.Visible := Not tv_AddbuildingName.Visible;
  tv_AddbuildingName.Top := Add.Top + ed_AddBuildingName.Top - tv_AddbuildingName.Height;
  tv_AddbuildingName.Left := Add.Left + ed_AddBuildingName.Left;
  if tv_AddbuildingName.Visible then tv_AddbuildingName.SetFocus;

end;

procedure TfmNodeManager.ed_NodeIpChange(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.ed_NodeIpKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TfmNodeManager.ed_SearchNameChange(Sender: TObject);
begin
  inherited;
  SearchList('');
end;

procedure TfmNodeManager.FormChangeEvent(aFormName: integer);
begin
//  case aFormName of
//  end;
end;

procedure TfmNodeManager.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  ...
  fmMain.FORMENABLE(con_FormBMOSNODEMANAGER,'FALSE');
...
procedure TfmNodeManager.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.FormGradeRefresh;
begin
  Try
...
procedure TfmNodeManager.FormIDSetting(aID: string);
begin
  L_stMenuID := aID;
end;

function TfmNodeManager.FormNameSetting: Boolean;
begin

  AdvOfficePage1.Caption := dmFormFunction.GetFormName('0','2','BUTTONMENU002');
  lb_List.Caption := dmFormFunction.GetFormName('2','1','BM5_021');//dmFormFunction.GetFormName('0','2','COMMONLIST01');
  ...
  btn_FireGroup.Caption := dmFormFunction.GetFormName('0','2','COMMONFIREGROUP');
  lb_AddFireGroup.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONFIREGROUP');
  btn_FireGroupAdd.Caption := dmFormFunction.GetFormName('0','2','COMMONADD');
  btn_FireGroupSelect.Caption := dmFormFunction.GetFormName('0','2','BUTTONAPPLY');
  btn_FireGroupCancel.Caption := dmFormFunction.GetFormName('0','2','BUTTONCANCEL001');
  lb_Serverno.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONSERVERNAME');

  lb_BuildingName.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONBUILDINGNAME');
  lb_SearchBuildingName.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONBUILDINGNAME');

end;

procedure TfmNodeManager.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.FormShow(Sender: TObject);
begin
  inherited;
  WindowState := wsMaximized;

  FormNameSetting;
  fmMain.FORMENABLE(con_FormBMOSNODEMANAGER,'TRUE');
...
procedure TfmNodeManager.pm_CodeAddClick(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.pm_UpdateCodeClick(Sender: TObject);
begin
  inherited;
  sg_ListDblClick(sg_List);
end;

function TfmNodeManager.SearchList(aCurrentCode:string;aTopRow:integer = 0): Boolean;
var
  stSql : string;
...
procedure TfmNodeManager.sg_ListCheckBoxClick(Sender: TObject; ACol,
...
procedure TfmNodeManager.sg_ListDblClick(Sender: TObject);
var
  nIndex : integer;
  vNode1 : TTreeNode;
  bNode : TTreeNode;
...
procedure TfmNodeManager.TimerProgressTimer(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.tv_AddbuildingNameDblClick(Sender: TObject);
begin
  inherited;
...
procedure TfmNodeManager.tv_buildingNameDblClick(Sender: TObject);
begin
  inherited;
...
function TfmNodeManager.UpdateNodeWork: Boolean;
var
  stNodeNo : string;
  stMessage : string;
  stDate : string;
...
begin
  ...
  dmDBUpdate.UpdateTB_ARMAREA_DefaultNodeArmAreaName(stNodeNo,ed_NodeName.Text,ed_AddBuildingCode.Text);
  dmDBUpdate.UpdateTB_DEVICE_DefaultNodeDeviceName(stNodeNo,ed_NodeName.Text,ed_AddBuildingCode.Text);
  dmDBUpdate.UpdateTB_DOOR_DefaultNodeDoorName(stNodeNo,ed_NodeName.Text,ed_AddBuildingCode.Text);
  //dmDBUpdate.UpdateTB_READER_DefaultNodeReaderName(stNodeNo,ed_NodeName.Text);
  dmDBUpdate.UpdateTB_ZONE_DefaultNodeZoneName(stNodeNo,ed_NodeName.Text,ed_AddBuildingCode.Text);
...
initialization
  RegisterClass(TfmNodeManager);
Finalization
  UnRegisterClass(TfmNodeManager);
