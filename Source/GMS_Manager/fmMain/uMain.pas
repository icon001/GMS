...
//  TfmMain = class(TForm)
  ...
  private
    ...
    procedure DeleteCardPermitDelete; //삭제된 카드 권한 삭제
...
begin
  ...
  if G_nBMSType = 3 then Exit;  //식수 모니터링인 경우는 알람 발생 안한다.
  if G_nBMSType = 4 then Exit;  //근태 모니터링인 경우는 알람 발생 안한다.
  ...
  begin
    L_nAccessSoundCount := 0;
    AccessSoundList.Delete(0); //다음 차례 Play
  end;
...
begin
  lb_AccessMessage.Caption.Text := ' 데이터베이스 접속 세션 연결상태를 확인해 주세요 !!! ';
  pan_Access.Visible := True;

  Exit;   //충칭에서 이 부분에서 세션을 끊으면서 권한이 적용 안되는 사례가 있었음
...
begin
  ...
  Try
    ...
    begin
      ...
      if G_nMonitoringType = 1 then G_bDataLoadingView := True; //분기국사는 데이터량이 많아서 로딩을 보여 줘야 한다.
...
begin
  ...
  Try
    ...
    Except
      Exit;  //Window 7 에서 권한 부분에 의해 write가 안됨
    End;
  Finally
    ini_fun.Free;
  End;
...
begin
  ...
  Try
    ...
    begin
      ...
      while Not Eof do
      begin
        dmDBCardPermit.EmployeePermitAllDelete(FindField('EM_SEQ').AsString); //사원 전체권한 삭제
        Next;
        Application.ProcessMessages;
      end;
    end;
...
begin
  ...
  begin
    if TdmPCClient(PCClientSocketList.Objects[nIndex]).GetArmAreaState(con_CMDArmAreaUsed,aNodeNo,aECUID,aArmArea) <> '1' then Exit;  //조회 방범 구역이 아니면 빠져 나가자...
  end;

  if G_bModeChangeSound then
  begin
    if copy(aAlarmStatePCCode,3,2) = 'A0' then   //경계모드 변경시
    begin
      if (MediaPlayer1.Mode <> mpPlaying) and (MediaPlayer2.Mode <> mpPlaying) then  //알람 울리고 있는 동안에는 소리 내지 말자.
      begin
        WindowsMediaPlayer1.Controls.stop;
        WindowsMediaPlayer1.URL := G_stModeChangeArmSoundFile;
        WindowsMediaPlayer1.Controls.play;
      end;
    end else if copy(aAlarmStatePCCode,3,2) = 'D0' then  //해제모드 변경시
    begin
...
begin
  ...
  begin
    ...
    if nIndex > -1 then  //식수리더에서 올라온 데이터이면
    begin
...
begin
  ...
  if G_bDoorOpenAlarmUse and (aCmd = SCCMDDOORDSSTATE) and (aData = 'O')then    //출입문이 열린 경우
  begin
    nIndex := OpenDoorAlarmList.IndexOf(FillZeroStrNum(aNodeNo,G_nNodeCodeLength) + aEcuID + aNo);
    if nIndex > -1 then
    begin
      //여기까지 온것은 관제 권한이 있는 것이니까 여기에서 메시지 출력 해 주면 된다.
  ...
  if G_bDoorLongTimeOpenEventUse and ((aData = 'D') or (aData = 'T')) then   //장시간 열림 상태 알람 울리면
  begin
    if bMonitoring then
    begin
...
//  if Not dmPCClient.Connected then Exit;
  ...
  Try
    Try
      ...
      if G_MonitoringNodeList.Count < 1 then Exit;
      if TMonitoringNode(G_MonitoringNodeList.Objects[L_nCurrentCheckDoorNodeSeq]).NodeConnected <> '0' then  //연결 되어 있으면 출입문 상태 체크 하자.
      begin
        ...
        begin
          if TMonitoringNode(G_MonitoringNodeList.Objects[L_nCurrentCheckDoorNodeSeq]).GetDeviceIndexConnect(j) = '1' then  //컨트롤러 연결 되어 있는데...
          begin
...
begin
  ...
  Try
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if nResult <> 0 then
            begin
              if nResult = -1 then stError := '카드번호미등록:' + EmployeeFileList.Strings[i]
              else if nResult = -2 then stError := '그룹코드미등록:' + EmployeeFileList.Strings[i]
              else if nResult = -3 then stError := '사원정보입력실패:' + EmployeeFileList.Strings[i]
              else if nResult = -4 then stError := '사원정보업데이트실패:' + EmployeeFileList.Strings[i]
              else stError := '기타에러('+inttostr(nResult) + '):' + EmployeeFileList.Strings[i];
    ...
    begin
      if FindCharCopy( EmployeeFileList.Strings[0],0,',') <> '부문' then
      begin
        showmessage('파일포맷이 틀립니다.업로드파일포맷 생성 후에 해당 포맷에 맞춰 업로드 하세요.');
        Exit;
      end;
    end;
    if EmployeeFileList.Count > 1 then
...
begin
  ...
  stATType := '0';
  stFDType := '0';
  stGubunCode := '0'; //직원구분 코드 협력사,정직원 등
  stFingerUse := '0'; //지문사용유무 0.미사용,1.사용
  stEmStateCode := '1'; //1.재직,2.퇴직
  stAWCode := '1';
  stCardUse := '0'; //0.카드미등록,1.카드등록
  bGradeUpdate := True;
  ...
  Try
    ...
    if stEmCode = '' then //사번으로 구분해서 업로드 하자.
    begin
    ...
    begin
      ...
      if aDupCardexcept then  //중복 카드 제외 업로드 이면 카드 체크해서 있으면 빠져 나간다.
      begin
    ...
    if dmDBFunction.GetTB_EMPLOYEE_EmSeq(stEMCode,stEmseq,stTemp1,stTemp2,stTemp3) <> 1 then  //사원정보 체크 해서 생성 하자.
    begin
    ...
    begin
      ...
      begin
        if (stGradeType = '1') and ( stTemp1 = stCompanyCode) then bGradeUpdate := False; //소속별 권한 이면서 소속이 변경 되지 않았으면 권한 재작업 할 필요 없다.
        if (stGradeType = '2') and ( stTemp2 = stGroupPermitCode) then bGradeUpdate := False; //그룹별 권한 이면서 그룹이 변경 되지 않았으면 권한 재작업 할 필요 없다.
      end;
    end;
    ...
    begin
      ...
      begin
        if stCardUse <> '1' then stCardUse := ''; //카드가 미등록 상태이면 그 전에 등록 상태일수도 있다.
      ...
      begin
        if stGradeType = '3' then  //개인별 권한 작업이면
        begin
          dmDBUpdate.UpdateTB_CARDPERMITEMPLOYEECODE_EmSeqApply(stEmSeq,'N','','',aEmApprove); //해당 사번의 권한 적용 부분을 재 작업 - 삭제된 카드가 있거나 추가된 카드가 있으므로
        end else
        begin
...
begin
  ...
  bGradeUpdate := True;
(*
  stSql := ' select b.CO_COMPANYNAME as ''부문'',c.CO_COMPANYNAME as ''본부'', ';
  stSql := stSql + ' d.CO_COMPANYNAME as ''지사/지점/센터'',e.CO_COMPANYNAME as ''팀'',';
  stSql := stSql + ' a.EM_CODE as ''사번'',a.EM_NAME as ''이름'',';
  stSql := stSql + ' f.EM_GUBUNCODENAME as ''직원구분'',';
  stSql := stSql + ' a.EM_HANDPHONE as ''핸드폰'',a.EM_COMPANYPHONE as ''사내전화번호'',';
  stSql := stSql + ' a.EM_EMINFO as ''기타정보'',';
  stSql := stSql + ' g.CA_PRINTNO as ''카드일련번호'',';
  stSql := stSql + ' g.CA_CARDNO as ''카드번호'', ';
  stSql := stSql + ' a.EM_ENDDATE as ''유효기간'' ';
*)
  LineDataList := TStringList.Create;
  Try
    ...
    LineDataList.DelimitedText := aLine;
    if LineDataList.Count < 7 then Exit;
    stCompanyName1 := LineDataList.Strings[0]; //부문
    stCompanyName2 := LineDataList.Strings[1]; //본부
    stCompanyName3 := LineDataList.Strings[2]; //지사/지점/센터
    stCompanyName4 := LineDataList.Strings[3]; //팀
    if aVer = 1 then  //1.0 버젼이면
    begin
      stEmCode := LineDataList.Strings[4];
...
begin
  ...
  L_bFormActivate := True; // 한번만 Activate 실행 되도록 막아줌
  ...
  begin
    ...
    if stKey = '' then
    begin
      if G_nFormLanguageType = 1 then showMessage('프로그램 설치후 프로그램 인증을 받으셔야 사용 가능 합니다.')
    ...
    if Not bResult then
    begin
      showmessage(dmFormMessage.GetMessage('INCORRECTAUTHKEY')); //인증키값이 틀립니다.')
    ...
    begin
      ...
      begin
        showmessage(dmFormMessage.GetMessage('EXPIRAUTHKEY'));
        //if G_nFormLanguageType = 1 then showmessage('인증키의 유효기간이 만료되었습니다.')
...
begin
  ...
  begin
    TdmPCClient(PCClientSocketList.Objects[i]).SendPacket('E',SCMONITORING,'A','0000','00',''); //알람 확인 전송
  end;
end;
...
(*
  if (G_nProgramType = 0) and FileExists(G_stExeFolder + '\ktt.ico') then    //프로젝트에서 아이콘 변경 수행
  begin
...
begin
  Try
    ...
    case aName of
      con_FormBMOSSECURITYMONITOR : begin  //방범 관제 조회시
        if bValue then LoadSecurityEvent;
      end;
      con_FormBMOSMERGEMONITOR : begin  //방범 관제 조회시
        if bValue then LoadSecurityEvent;
      end;
      con_FormBMOSMAPSECURITYMONITOR : begin  //방범맵 관제 조회시
        if bValue then LoadSecurityEvent;
      end;
      con_FormBMOSMAPSECURITYACCESS : begin  //방범맵 관제 조회시
        if bValue then LoadSecurityEvent;
      end;
      con_FormBMOSMAPACCESS : begin  //출입맵 관제 조회시
        //if bValue then LoadSecurityEvent;
      end;
...
//    G_nBuildingStep := ini_fun.ReadInteger('Config','Building',1);
  ...
  if G_nMonitoringType = 2 then //SK 이면 caption 을 ACAM_Manager 로 바꾸자.
  begin
...
//  dmPCClient.ServerPort := strtoint(G_stDaemonControlPort);
  ...
  if G_nMonitoringType = 1 then  //분기국사 타입인 경우
  begin
    mn_DoorCode.Visible := False;
    mn_Holiday.Visible := False; //공휴일 관리는 보이지 말자.
    mn_TimeCode.Visible := False; //타임코드 관리는 보이지 말자.
    mn_ArmAreaCode.Visible := False; //방범구역 관리는 보이지 말자.
    mn_Zone.Visible := False;       //존관리도 보이지 말자.
    mn_MapAdmin.Visible := False;   //맵관리도 보이지 말자.
    btn_PosiCode.Visible := False;   //직위코드 관리 보이지 말자.
    ...
    G_bAccessMonitoringUse := True;
    G_bAlarmMonitoringUse := False;   //분기국사는 방범구역 관련 된 내역이 안보인다.
    mn_TelAccessReport.Visible := True;
  end else
  begin
    mn_DoorCode.Visible := True;
    mn_Holiday.Visible := True; //공휴일 관리
    mn_TimeCode.Visible := True; //타임코드 관리
    mn_ArmAreaCode.Visible := True; //방범구역 관리
    mn_Zone.Visible := True;       //존관리
    mn_MapAdmin.Visible := True;   //맵관리
    btn_PosiCode.Visible := True;   //직위코드
  ...
  end else if G_nBMSType = 3 then  //근태
  begin
  end else if G_nBMSType = 4 then  //식수
  begin
...
(*
  ...
  begin
    DeleteCardPermitDelete;   //삭제된 인원 중 카드권한 삭제되지 않은 사람 모두 권한삭제
  end;
  dmDBUpdate.UpdateTB_CONFIG_Field_StringValue('ACAM','VER','CO_CONFIGVALUE','2');

  TravelToolBarImageChange(at_Menu); //메뉴 Image Change
  ...
  begin
    showmessage('서버 버젼과 현재 버젼이 호환되지 않습니다. 업그레이드 후 사용하세요');
    Application.Terminate;
  end;
end;
  ...
  begin
    ...
    begin
      if TdmPCClient(PCClientSocketList.Objects[i]).Connected then
        TdmPCClient(PCClientSocketList.Objects[i]).SendPacket('C',SCMONITORING,'R','0000','00','DAEMON'); //데몬 재시작
    end;
  end;
...
begin
  ...
  if G_nMonitoringType = 1 then   //분기국사용 사원관리
  begin
...
begin
  ...
  Try
    ...
    begin
      ...
      strCsvRec := '부문,본부,지사/지점/센터,팀,사번,이름,직원구분,핸드폰,사내전화번호,기타정보,카드일련번호,카드번호';
...
begin
  ...
  Try
    ...
    begin
      stHeader := '부문,';
      stHeader := stHeader + '본부,';
      stHeader := stHeader + '지사/지점/센터,';
      stHeader := stHeader + '팀,';
      stHeader := stHeader + '사번,';
      stHeader := stHeader + '이름,';
      stHeader := stHeader + '직원구분,';
      stHeader := stHeader + '핸드폰,';
      stHeader := stHeader + '사내전화번호,';
      stHeader := stHeader + '협력사명,';
      stHeader := stHeader + '카드일련번호,';
      stHeader := stHeader + '카드번호(숫자10자리),';
      stHeader := stHeader + '유효기간(yyyymmdd),';
      stHeader := stHeader + '권한타입(1.소속별권한;2.그룹별권한;3.개인별권한),';
      stHeader := stHeader + '권한그룹명';
      FileFormatList.Add(stHeader);
    end else
    begin
      stHeader := '소속1,';
      stHeader := stHeader + '소속2,';
      stHeader := stHeader + '소속3,';
      stHeader := stHeader + '소속4,';
      stHeader := stHeader + '소속5,';
      stHeader := stHeader + '소속6,';
      stHeader := stHeader + '직위명,';
      stHeader := stHeader + '사번,';
      stHeader := stHeader + '이름,';
      stHeader := stHeader + '직원구분,';
      stHeader := stHeader + '핸드폰,';
      stHeader := stHeader + '사내전화번호,';
      stHeader := stHeader + '기타정보,';
      stHeader := stHeader + '카드번호,';
      stHeader := stHeader + '유효기간(yyyymmdd),';
      stHeader := stHeader + '권한타입(1.소속별권한;2.그룹별권한;3.개인별권한),';
      stHeader := stHeader + '권한그룹명';
      FileFormatList.Add(stHeader);
...
//  end;
  ...
  begin
    ...
    begin
      if TdmPCClient(PCClientSocketList.Objects[i]).Connected then
        TdmPCClient(PCClientSocketList.Objects[i]).SendPacket('C',SCMONITORING,'F',FillZeroNumber(0,G_nNodeCodeLength),'00',''); //화재복구
    end;
  end;
end;
...
begin
  ...
  begin
    L_nOpenDoorSoundCount := 0;
    OpenDoorAlarmSoundList.Delete(0); //다음 차례 Play
  end;
  MediaPlayer2.FileName := stAccessFile;
  L_bSoundTimerEnable := True;
  Try
...
begin
  ...
  //DeviceStateTimer.Enabled := aConnected;  //소켓 접속 되면 상태 조회
...
//  if FLogined = Value then Exit;
  ...
  begin
//    if G_nBMSType = 2 then apAdmin.TabVisible := G_bIsMaster;   //출입모니터링에서만 관리자 관리 하자.
    ...
    begin
      ...
      Application.ProcessMessages;
//      if Not G_bApplicationTerminate then LoadingTB_DECODER;   //해당 권한 노드만 띄우자..
...
begin
  ...
  begin
    L_nAlarmSoundCount := 0;
    AlarmSoundList.Delete(0); //다음 차례 Play
  end;
