...
begin
  ...
  if ed_cmd2.Text <> '' then
  begin
    if Pos(stCmd2,ed_cmd2.Text) = 0 then Exit;
  end;

  DisplayList.Add(inttostr(aNodeNo) + ',' + aEcuID + aCmd + aMsgNo + aType +',' + aDeviceVer + ',' + aData + ',' + aNodeName );
...
var
  stTemp : string;
  stNodeNo : string;
  stEcuID,stCmd,stMsgNo,stType : string;
  stDeviceVer : string;
  stData : string;
  stNodeName : string;
begin
  ...
  if Not L_bStart then
  begin
    DisplayList.Clear;
    Exit;
  end;

  if DisplayList.Count < 1 then Exit;
  stTemp := DisplayList.Strings[0];
  DisplayList.Delete(0);
  stNodeNo := FindCharCopy(stTemp,0,',');
  stEcuID := FindCharCopy(stTemp,1,',');
  stType := copy(stEcuID,5,2);
  stMsgNo := stEcuID[4];
  stCmd := stEcuID[3];
  stEcuID := copy(stEcuID,1,2);
  stDeviceVer := FindCharCopy(stTemp,2,',');
  stData := FindCharCopy(stTemp,3,',');
  stNodeName := FindCharCopy(stTemp,4,',');
  ...
  begin
    ...
    Cells[0,1] := stNodeName;
    Cells[1,1] :=  stEcuID ;
    Cells[2,1] :=  FormatDateTime('hh:nn:ss',now) ;
    Cells[3,1] :=  stType ;
    Cells[4,1] :=  stCmd ;
    Cells[5,1] :=  stMsgNo ;
    Cells[6,1] :=  stDeviceVer ;
    Cells[7,1] :=  stData ;
    if UpperCase(stType) = 'RX' then
    begin
      RowColor[1] := $00EACAB6;
    end;
    if UpperCase(stType) = 'ER' then RowColor[1] := clYellow;
