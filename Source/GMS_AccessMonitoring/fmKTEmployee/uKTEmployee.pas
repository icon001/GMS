...
type
  TfmKTEmployee = class(TfmASubForm)
    BodyPanel: TW7Panel;
    ...
    ed_CardPrintNo: TAdvEdit;
    AdvGlowButton4: TAdvGlowButton;
    tv_AddEmGroupName: TTreeView;
    lb_List: TAdvSmoothLabel;
    lb_EmployeeList: TAdvSmoothLabel;
    lb_EmployeeCaption: TAdvSmoothLabel;
    pan_ValidDateManage: TAdvSmoothPanel;
    lb_ValidDateCaption: TLabel;
    ...
    lb_emgradecaption: TAdvSmoothLabel;
    ...
    procedure ed_CardPrintNoKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  private
    ...
    procedure SetCardRegistView(const Value: Boolean);
    procedure SetPrintNo(const Value: string);
    procedure SetEMSEQ(const Value: integer);
    procedure SetCardGrade(const Value: Boolean);
  public
    { Public declarations }
  ...
  public
    property EMSEQ : integer read FEMSEQ write SetEMSEQ;
    property CardNo : string read FCardNo write SetCardNo;
    property PrintNo : string read FPrintNo write SetPrintNo;
    property CardGrade : Boolean read FCardGrade write SetCardGrade;
    property CardRegistView : Boolean read FCardRegistView write SetCardRegistView;
  end;

var
  fmKTEmployee: TfmKTEmployee;
...
procedure TfmKTEmployee.ed_etcSearchChange(Sender: TObject);
begin
  inherited;
  if G_nReportSearchType = 1 then SearchEmployeeList('');
end;

procedure TfmKTEmployee.ed_etcSearchKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #13 then
  begin
...
procedure TfmKTEmployee.btn_emGradeCompleteClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.AdvGlowButton3Click(Sender: TObject);
var
  stFile : string;
begin
  inherited;
...
procedure TfmKTEmployee.AdvStrinGridSetAllCheck(Sender: TObject;
  bchkState: Boolean);
var
  i : integer;
begin
...
procedure TfmKTEmployee.btn_BuildingCancelClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_BuildingNameSelectClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_CancelClick(Sender: TObject);
begin
  inherited;
  Pan_Emcode.Visible := False;
  if CardRegistView then Close;

end;

procedure TfmKTEmployee.btn_CardAddClick(Sender: TObject);
begin
  inherited;
  CardAdd(True);
end;

procedure TfmKTEmployee.btn_CardGroupSelectClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_CloseClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TfmKTEmployee.btn_CompanyManageClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_CompanyNameSelectClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_CompanySelectClick(Sender: TObject);
var
  stMessage : string;
...
procedure TfmKTEmployee.btn_EmGroupCancelClick(Sender: TObject);
begin
  inherited;
  AdvSmoothPanel2.Enabled := True;
  List.Enabled := True;
...
procedure TfmKTEmployee.btn_EmGroupSelectClick(Sender: TObject);
var
  stMessage : string;
...
procedure TfmKTEmployee.btn_EmployeeAddClick(Sender: TObject);
begin
  inherited;
  pm_EmployeeAddClick(pm_EmployeeAdd);
end;

procedure TfmKTEmployee.btn_ExcelClick(Sender: TObject);
var
...
procedure TfmKTEmployee.btn_FingerCancelClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_FingerSaveClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_GroupManageClick(Sender: TObject);
begin
  inherited;
  if L_nEmployeeCheckCount < 1 then
  begin
...
procedure TfmKTEmployee.btn_minimizeClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_PermitArmAreaAddClick(Sender: TObject);
var
  stMessage : string;
  i : integer;
  bCheckState : Boolean;
...
procedure TfmKTEmployee.btn_PermitArmAreaDeleteClick(Sender: TObject);
var
  stMessage : string;
  i : integer;
  bCheckState : Boolean;
...
procedure TfmKTEmployee.btn_PermitDoorAddClick(Sender: TObject);
var
  stMessage : string;
  i : integer;
  bCheckState : Boolean;
...
begin
  ...
  pan_progress.Visible := True;
  ProgressBar1.Position := 0;
  ProgressBar1.Max := sg_NotPermitDoor.RowCount - 1;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        lb_ProgressCount.Caption.text := Cells[2,i] + Cells[3,i] + Cells[4,i] + Cells[5,i] + ':Permit Add';
        ...
        lb_ProgressCount.Caption.text := Cells[2,i] + Cells[3,i] + Cells[4,i] + Cells[5,i] + ':TB_SYSTEMLOG Insert';
...
procedure TfmKTEmployee.btn_PermitDoorDeleteClick(Sender: TObject);
var
  stMessage : string;
  i : integer;
  bCheckState : Boolean;
...
procedure TfmKTEmployee.btn_SaveClick(Sender: TObject);
var
  stMessage : string;
  stParentCode : string;
...
{  if rg_TimeGroup.ItemIndex = 0 then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          dmDBInsert.InsertIntoTB_CARD_Value(stCardNo,stCardState,stCardGubun,stPositionNum,stRelayID,'0',stEmSeq,stCardPrintNo);
        ...
        begin
          ...
          begin
            ...
            dmDBUpdate.UpdateTB_CARD_Value(stCardNo,stCardState,stCardGubun,stEmSeq,stCardPrintNo);
          end else
          begin
            //여기는 수정시에 처리 하는 루틴
            dmDBUpdate.UpdateTB_CARD_Value(stCardNo,stCardState,stCardGubun,stEmSeq,stCardPrintNo);
  ...
  begin
    ...
    begin
      stFingerCardNo := dmDBFunction.GetTB_CARD_FingerCardNo;      //Finger Card 한장 등록 하자.
    ...
    dmDBInsert.InsertIntoTB_EMPLOYEE_Value(stEmSeq,stEmCode,stEmName,stCompanyCode,stPosiCode,stGubunCode,
               ...
               FormatDateTime('yyyymmdd',dt_Outdate.Date),'1',stATType,stFdType,stEmGroupCode,stCardGroupType,stFingerUse,stEmStateCode,stAWCode,stCardUse);
...
//        dmDBCardPermit.EmployeeCardAllPermit(stEmSeq,con_ComLogTYPE_ARMAREA,'0','Y');
  ...
  Pan_Emcode.Visible := False;
  //if CardRegistView then Close  //카드등록 화면에서는 카드만 등록하고 빠져 나가자.
  //else
  cmb_EtcSearch.ItemIndex := 0;
  ed_etcSearch.Text := ed_EmCode.Text;
  SearchEmployeeList(stEmSeq,sg_EmployeeList.TopRow);
end;

procedure TfmKTEmployee.btn_SaveMouseEnter(Sender: TObject);
begin
  inherited;
  TAdvGlassButton(Sender).ShineColor := clYellow;
end;

procedure TfmKTEmployee.btn_SaveMouseLeave(Sender: TObject);
begin
  inherited;
  TAdvGlassButton(Sender).ShineColor := clGray;
end;

procedure TfmKTEmployee.btn_SearchClick(Sender: TObject);
begin
  inherited;
  SearchEmployeeList('');
end;

procedure TfmKTEmployee.btn_ValidCancelClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.btn_ValidDateManageClick(Sender: TObject);
begin
  inherited;
  if L_nEmployeeCheckCount < 1 then
  begin
  ...
  List.Enabled := False;
  lb_ValidDateCaption.Caption := btn_ValidDateManage.Caption;
  //pan_Company.Top := btn_CompanyManage.Top - pan_Company.Height - 20;
  pan_ValidDateManage.Visible := True;
  dt_ValidDate.Date := now;
end;

procedure TfmKTEmployee.btn_ValidSaveClick(Sender: TObject);
var
  stMessage : string;
  i : integer;
  bCheckState : Boolean;
...
procedure TfmKTEmployee.CardAdd(aButton: Boolean);
var
...
(*          RegistTempDB(G_stCardRegisterIP);
  ...
  if dmDBFunction.GetTB_CARDPRINTLIST_PrintNoFromCardNo(ed_RegCardNo.Text) = '' then
     dmDBInsert.InsertIntoTB_CARDPRINTLIST_Value(ed_CardPrintNo.Text,ed_RegCardNo.Text);
...
//  else if G_nCardNoType = 2 then ed_RegCardNo.Text := ed_RegCardNo.Text + '0**';//KT사옥타입
  ...
  begin
    ...
    cells[6,nRow] := ed_CardPrintNo.Text;
  end;
  L_bEmployeeGradeChange := True;   //추가되면 권한 작업 재정리 해 줘야 함
  ed_RegCardNo.Text := '';
end;

procedure TfmKTEmployee.CardRegistEvent(aCardNo: string);
begin
...
function TfmKTEmployee.Check_FormCritical(var aBlankName: string): Boolean;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
begin
...
procedure TfmKTEmployee.chk_AddAutoEmpCodeClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.chk_FingerUseClick(Sender: TObject);
var
  nResult : UF_RET_CODE;
  i,j : integer;
  nLen : integer;
...
procedure TfmKTEmployee.chk_IncludeCardNumClick(Sender: TObject);
begin
  inherited;
  SearchEmployeeList('');
end;

procedure TfmKTEmployee.cmb_CardWorkChange(Sender: TObject);
var
  stTemp: string;
  stMessage : string;
begin
...
procedure TfmKTEmployee.cmb_CardWorkExit(Sender: TObject);
begin
  inherited;
  cmb_CardWork.hide;
end;

procedure TfmKTEmployee.cmb_EmCardGradeTypeChange(Sender: TObject);
begin
  inherited;
  L_bEmployeeGradeChange := True;
...
procedure TfmKTEmployee.cmb_EmpGubunSearchClick(Sender: TObject);
begin
  inherited;
  if G_nReportSearchType = 1 then SearchEmployeeList('');
end;

procedure TfmKTEmployee.cmb_EtcSearchChange(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.cmb_PositionSearchClick(Sender: TObject);
begin
  inherited;
  if G_nReportSearchType = 1 then SearchEmployeeList('');
end;

procedure TfmKTEmployee.dt_IndateChange(Sender: TObject);
begin
  inherited;
  L_bEmployeeGradeChange := True;  //유효기간 변경시 카드데이터 재전송
end;

procedure TfmKTEmployee.dt_OutDateChange(Sender: TObject);
begin
  inherited;
  L_bEmployeeGradeChange := True; //유효기간 변경시 카드데이터 재전송
end;

procedure TfmKTEmployee.ed_CardPrintNoKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  ed_RegCardno.Text := dmDBFunction.GetTB_CARDPRINTLIST_CardNo(ed_CardPrintNo.Text);
  //
end;

procedure TfmKTEmployee.ed_CompanyNameKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #13 then
  begin
...
procedure TfmKTEmployee.ed_EmCodeExit(Sender: TObject);
var
...
begin
  ...
  if dmDBFunction.checkTB_EMPLOYEE_EMCODE_Variable(ed_EmCode.Text,stEmSeq,stACType,stATType,stFDType,stEmName) = 1 then
  begin
...
procedure TfmKTEmployee.employeePanelClear;
begin
  ...
  sg_Card.ColWidths[4] := 0;
  sg_Card.ColWidths[5] := 0;
...
function TfmKTEmployee.EmployeePermitToCardDeviceLoad(aNodeNo, aEcuID,
...
function TfmKTEmployee.EmployeePermitToCardLoad(aEmSeq: string): Boolean;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
begin
...
procedure TfmKTEmployee.FingerTimerTimer(Sender: TObject);
begin
  inherited;
  lb_FingerMessage.Visible := Not lb_FingerMessage.Visible;
end;

procedure TfmKTEmployee.FontSetting;
begin
...
procedure TfmKTEmployee.FormActivate(Sender: TObject);
begin
  inherited;
  if CardGrade then
  begin
    ...
    ed_etcSearch.Text := CardNo;
    btn_SearchClick(btn_Search);
    sg_EmployeeList.SelectRows(1,1);
    sg_EmployeeList.Row := 1;
    Pan_Emcode.Visible := False;
    pm_EmployeeGradeAdminClick(pm_EmployeeGradeAdmin);
  end;

end;

procedure TfmKTEmployee.FormChangeEvent(aNumber: integer);
var
...
procedure TfmKTEmployee.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TfmKTEmployee.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.FormGradeRefresh;
begin
...
procedure TfmKTEmployee.FormIDSetting(aID: string);
begin
  L_stMenuID := aID;
end;

procedure TfmKTEmployee.FormNameSetting;
begin
  AdvOfficePage1.Caption := dmFormFunction.GetFormName('0','2','BUTTONMENU002');
  lb_List.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONCOMPANYLIST');//소속 dmFormFunction.GetFormName('0','2','COMMONLIST01');
  ...
  lb_EmployeeList.Caption.Text := dmFormFunction.GetFormName('2','2','BM3_011List');
  if G_nCardRegisterType = 1 then
...
//  pan_PositionInfo.Caption.Text := dmFormFunction.GetFormName('0','2','COMMONBUILDINGINFO');
  ...
  sg_EmployeeList.Cells[16,0]:= dmFormFunction.GetFormName('0','2','COMMONEMSEQ');
  sg_EmployeeList.Cells[17,0]:= dmFormFunction.GetFormName('0','2','COMMONCOMPANY');
  sg_EmployeeList.Cells[18,0]:= dmFormFunction.GetFormName('0','2','COMMONEMPLOYEEPOSI');
  sg_EmployeeList.Cells[19,0]:= dmFormFunction.GetFormName('0','2','COMMONEMPLOYEEGUBUN');
  sg_EmployeeList.Cells[20,0]:= dmFormFunction.GetFormName('0','2','COMMONEMPLOYEEGROUP');
  sg_EmployeeList.Cells[21,0]:= dmFormFunction.GetFormName('0','2','COMMONCARDGRADE');
...
//  lb_NotDoorPermit.Caption := dmFormFunction.GetFormName('0','2','COMMONDOORNOTPERMIT');
  ...
  cmb_EtcSearch.Items.Add(L_stCardPrintNoCaption);
...
procedure TfmKTEmployee.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.FormShow(Sender: TObject);
begin
  inherited;
  ...
  tab_AlarmPermit.TabVisible := G_bAlarmMonitoringUse;
  if CardRegistView then pm_EmployeeAddClick(pm_EmployeeAdd);
end;

procedure TfmKTEmployee.Form_Close;
begin
  Close;
end;

procedure TfmKTEmployee.Image1Click(Sender: TObject);
begin
  inherited;
...
function TfmKTEmployee.JpegStartsInBlob(PicField: TBlobField): integer;
var
  bS : TADOBlobStream;
  buffer : Word;
  hx : string;
...
procedure TfmKTEmployee.LoadBuildingAlarmAll(aBuildingCode: string;
...
procedure TfmKTEmployee.LoadBuildingDoorAll(aBuildingCode: string;
  sgList: TAdvStringGrid);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
...
procedure TfmKTEmployee.LoadPermitAlarm(aBuildingCode, aEmSeq: string);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
  nRow : integer;
...
procedure TfmKTEmployee.LoadPermitDoor(aBuildingCode, aEmSeq: string);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
  nRow : integer;
...
procedure TfmKTEmployee.pm_EmployeeGradeAdminClick(Sender: TObject);
begin
  inherited;
  ...
  lb_emgradecaption.Caption.text := pm_EmployeeGradeAdmin.Caption;
  //tp_Grade.Align := alClient;
  PageControl1.ActivePageIndex := 0;
  tv_buildingNameDblClick(self);
end;

procedure TfmKTEmployee.NotPermitArmAreaListAdd(aArmAreaName, aNodeNo, aEcuID,
...
procedure TfmKTEmployee.NotPermitArmAreaListDelete(aArmAreaCode: string);
var
  i : integer;
begin
  with sg_NotPermitAlarm do
...
procedure TfmKTEmployee.NotPermitDoorListAdd(aDoorName, aNodeNo, aEcuID,
...
procedure TfmKTEmployee.NotPermitDoorListDelete(aDoorCode: string);
var
  i : integer;
begin
...
procedure TfmKTEmployee.pan_AddWorkResize(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.Pan_EmcodeMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TfmKTEmployee.Pan_EmcodeMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
const
 SC_DragMove = $F012;
begin
...
procedure TfmKTEmployee.Pan_EmcodeMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
    // MOUSE DOWN = FALSE 설정
...
procedure TfmKTEmployee.pan_emPermitAdminMouseDown(Sender: TObject;
...
procedure TfmKTEmployee.pan_EmployeeListResize(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.pan_FingerMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
const
 SC_DragMove = $F012;
begin
...
procedure TfmKTEmployee.pan_NotPermitAlarmAreaResize(Sender: TObject);
begin
  inherited;
  sg_NotPermitAlarm.Width := pan_NotPermitAlarmArea.Width - sg_NotPermitAlarm.Left - 20;
end;

procedure TfmKTEmployee.pan_NotPermitDoorResize(Sender: TObject);
begin
  inherited;
  sg_NotPermitDoor.Width := pan_NotPermitDoor.Width - sg_NotPermitDoor.Left - 20;
end;

procedure TfmKTEmployee.pan_PermitAlarmAreaResize(Sender: TObject);
begin
  inherited;
  sg_PermitAlarm.Width := pan_PermitAlarmArea.Width - sg_PermitAlarm.Left - 20;
end;

procedure TfmKTEmployee.pan_PermitDoorResize(Sender: TObject);
begin
  inherited;
  sg_PermitDoor.Width := pan_PermitDoor.Width - sg_PermitDoor.Left - 20;
end;

procedure TfmKTEmployee.pan_PositionInfoMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
const
 SC_DragMove = $F012;
begin
...
procedure TfmKTEmployee.PermitArmAreaListAdd(aArmAreaName, aNodeNo, aEcuID,
  aExtendID, aArmAreaNo, aArmAreaCode, aArmAreaView: string);
var
  i : integer;
  bAdd : Boolean;
...
procedure TfmKTEmployee.PermitDoorListAdd(aDoorName, aNodeNo, aEcuID, aExtendID,
...
procedure TfmKTEmployee.pm_EmDeleteClick(Sender: TObject);
var
  stMessage : string;
  stEmSeq : string;
  stSql : string;
...
procedure TfmKTEmployee.pm_EmployeeAddClick(Sender: TObject);
var
  stMessage : string;
  nIndex : integer;
begin
  inherited;
  if Not CardRegistView and (tv_List.Selected = nil) then
  begin
  ...
  lb_EmployeeCaption.Caption.Text := pm_EmployeeAdd.Caption;
  if Not CardRegistView then ed_SelectCompanyCode.Text := tv_Code.Items.Item[tv_List.Selected.AbsoluteIndex].Text
  else ed_SelectCompanyCode.Text := '0';
  LoadPosiCode(copy(ed_SelectCompanyCode.Text,1,1+G_nCompanyCodeLength) ,PosiCodeList,cmb_PosiName);
  if Not CardRegistView then ed_CompanyName.Text := tv_List.Selected.Text
  else ed_CompanyName.Text := '';
...
procedure TfmKTEmployee.pm_EmUpdateClick(Sender: TObject);
var
  stMessage : string;
  nIndex : integer;
  stTemp : string;
...
begin
  ...
  employeePanelClear;
  Pan_Emcode.Visible := True;
  lb_EmployeeCaption.Caption.Text := pm_EmUpdate.Caption;
  with sg_EmployeeList do
  begin
...
procedure TfmKTEmployee.pm_GroupPermitClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.RegistTempDB(G_stCardRegisterIP: string);
var
  stSql : string;
begin
...
procedure TfmKTEmployee.SearchCardList(aEmSeq: string);
var
  stSql : string;
  nRow : integer;
  stCardGubun : string;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          cells[6,nRow] := FindField('CA_PRINTNO').AsString;
        end;
        nRow := nRow + 1;
        Next;
      end;
...
procedure TfmKTEmployee.SearchEmGroupList(aCode: string);
var
  aTreeView : TTreeview;
  vTreeView : TTreeview;
...
procedure TfmKTEmployee.SearchEmployeeList(aCode: string; aTopRow: integer);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
  nRow : integer;
...
begin
  ...
  if CardGrade then stCompanyCode := '0'; //카드권한 부여시에는 전체 소속을 모두 조회 하자.
  ...
  stCardNo := '';
  if cmb_EtcSearch.Text = L_stCardNoCaption  then
  begin
    stCardNo := ed_etcSearch.Text;
  end;
//  L_stCardPrintNoCaption := '';
  if cmb_EtcSearch.Text = L_stCardPrintNoCaption  then
  begin
    if ed_etcSearch.Text <> '' then
    begin
      stCardNo := dmDBFunction.GetTB_CARDPRINTLIST_CardNo(ed_etcSearch.Text);
      if stCardNo = '' then Exit;  //없는 카드번호이다
    end;
  end;

  if G_nBMSType = 2 then
    stSql := dmDBSelect.SelectTB_EMPLOYE_Value(stCompanyCode,stPosiCode,'',stEmCode,stEmName,stEmployeeGubun,stEmHandPhone,stEmCompanyPhone,stEtcInfo,'','','1','','',chk_IncludeCardNum.Checked,'','',stCardNo)  //출입타입
  else if G_nBMSType = 3 then
    stSql := dmDBSelect.SelectTB_EMPLOYE_Value(stCompanyCode,stPosiCode,'',stEmCode,stEmName,stEmployeeGubun,stEmHandPhone,stEmCompanyPhone,stEtcInfo,'','','','1','',chk_IncludeCardNum.Checked,'','',stCardNo)  //근태타입
  else if G_nBMSType = 4 then
    stSql := dmDBSelect.SelectTB_EMPLOYE_Value(stCompanyCode,stPosiCode,'',stEmCode,stEmName,stEmployeeGubun,stEmHandPhone,stEmCompanyPhone,stEtcInfo,'','','','','1',chk_IncludeCardNum.Checked,'','',stCardNo)  //식수타입
  ;
...
(*
procedure TfmKTEmployee.SearchMEmGroupList(aCode: string);
var
  aTreeView : TTreeview;
  vTreeView : TTreeview;
  aNode,bNode,cNode : TTreeNode;
...
procedure TfmKTEmployee.SearhAddEmployee(aEmSeq: string);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
  stTemp : string;
...
procedure TfmKTEmployee.SetCardGrade(const Value: Boolean);
begin
  FCardGrade := Value;

end;

procedure TfmKTEmployee.SetCardNo(const Value: string);
begin
  FCardNo := Value;
  ed_RegCardNo.Text := Value;
  ed_CardPrintNo.Text := dmDBFunction.GetTB_CARDPRINTLIST_PrintNoFromCardNo(Value);
  EMSEQ := dmDBFunction.GetTB_CARD_EmSeqFromCardNo(Value);
  if ed_CardPrintNo.Text = '' then
  begin
    ed_CardPrintNo.Text := dmDBFunction.GetTB_CARDPRINTLIST_AutoPrintNo;
  end;
  ed_CardPrintNo.Enabled := Not CardRegistView;
end;

procedure TfmKTEmployee.SetCardRegistView(const Value: Boolean);
begin
  FCardRegistView := Value;
end;

procedure TfmKTEmployee.SetEMSEQ(const Value: integer);
begin
  if FEMSEQ = Value then Exit;

  FEMSEQ := Value;
  if Value = 0 then ed_emseq.text := ''
  else
  begin
    ed_emseq.text := inttostr(Value);

  end;
end;

procedure TfmKTEmployee.SetPrintNo(const Value: string);
begin
  FPrintNo := Value;
end;

procedure TfmKTEmployee.sg_CardButtonClick(Sender: TObject; ACol, ARow: Integer);
var
  stMessage : string;
  nIndex : integer;
begin
...
procedure TfmKTEmployee.sg_CardCanEditCell(Sender: TObject; ARow, ACol: Integer;
  var CanEdit: Boolean);
begin
  if Not L_bVisitEditMode then Exit;
...
procedure TfmKTEmployee.sg_CardSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
var
  nIndex : integer;
  Rect: TRect;
...
procedure TfmKTEmployee.sg_EmployeeListCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  inherited;
...
procedure TfmKTEmployee.sg_EmployeeListClick(Sender: TObject);
begin
  inherited;
  with sg_EmployeeList do
  begin
...
procedure TfmKTEmployee.sg_EmployeeListDblClick(Sender: TObject);
begin
  inherited;
  pm_EmUpdateClick(self);
end;

procedure TfmKTEmployee.sg_EmployeeListResize(Sender: TObject);
begin
  inherited;
  L_nPageListMaxCount := sg_EmployeeList.Height div sg_EmployeeList.DefaultRowHeight;
end;

procedure TfmKTEmployee.sg_NotPermitAlarmCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  inherited;
  if ARow = 0 then //전체선택 또는 해제
...
procedure TfmKTEmployee.sg_NotPermitDoorCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  inherited;
  if ARow = 0 then //전체선택 또는 해제
...
procedure TfmKTEmployee.sg_PermitAlarmCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  inherited;
  if ARow = 0 then //전체선택 또는 해제
...
procedure TfmKTEmployee.sg_PermitDoorCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  inherited;
  if ARow = 0 then //전체선택 또는 해제
...
procedure TfmKTEmployee.SKGasCardMapping(aFile: string);
var
...
procedure TfmKTEmployee.tv_AddCompanyNameDblClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.tv_AddEmGroupNameDblClick(Sender: TObject);
begin
  inherited;
...
procedure TfmKTEmployee.tv_buildingNameDblClick(Sender: TObject);
begin
  inherited;
  ...
  if G_bAlarmMonitoringUse then LoadBuildingAlarmAll(ed_SelectBuildingCode.Text,sg_NotPermitAlarm); //전체 방범구역을 리스트에 추가 하자

  LoadPermitDoor(ed_SelectBuildingCode.Text,ed_emGradeSeq.Text);
  if G_bAlarmMonitoringUse then LoadPermitAlarm(ed_SelectBuildingCode.Text,ed_emGradeSeq.Text);
(*
  if tv_buildingName.Selected = nil then tv_buildingName.Items[0].Selected := True;
...
procedure TfmKTEmployee.tv_ListClick(Sender: TObject);
var
  stCode : string;
begin
  inherited;
  ...
  lb_EmployeeList.Caption.Text := tv_List.Selected.Text;
...
procedure TfmKTEmployee.tv_ListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
...
procedure TfmKTEmployee.tv_ListDragOver(Sender, Source: TObject; X,
...
procedure TfmKTEmployee.ViewTimeGroupCode(aTimeGroup: string);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
begin
...
initialization
  RegisterClass(TfmKTEmployee);
Finalization
  UnRegisterClass(TfmKTEmployee);
