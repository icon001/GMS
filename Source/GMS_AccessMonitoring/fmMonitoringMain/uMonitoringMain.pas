...
type
  ...
  private
    ...
    L_arrAlarmEventSizeTable : Array of integer;   //알람이벤트 Cell Size
    ...
    L_nFormDeep : integer; //0.메인,1.모니터링
...
begin
  ...
  begin
    showmessage(stNodeIP + ':' + '데이터 타입 오류 입니다.');
    Exit;
  end;
...
begin
  ...
  if dmDBFunction.CheckTB_CARDPERMIT_DeviceCardNo(aNodeNo,aEcuID,'0',aCardNO) = 1 then  //이미 존재 하는 경우
  begin
...
begin
  ...
  begin
    ...
    if stDoorName = '' then
    begin
      if Not G_bIsMaster then Exit; //마스터가 아니면 무조건 빠져 나가자.
      stDoorName := stNodeIp;
    end;
  end else
  begin
...
begin
  ...
  begin
    ...
    Cells[L_arrAccessEventIndexArray[12],1] := aChangeState;   //타입
    Cells[L_arrAccessEventIndexArray[13],1] := stNodeNo;   //노드번호
    Cells[L_arrAccessEventIndexArray[14],1] := aEcuID;   //노드번호
    Cells[L_arrAccessEventIndexArray[15],1] := aDoorNo;   //출입문번호
    Cells[L_arrAccessEventIndexArray[16],1] := aAccessResult[1];   //출입승인코드
...
//  Perform( WM_SYSCOMMAND, $F012, 0 );
  Try
    ...
    L_stTitle := ini_fun.ReadString('Monitoring','Caption','분기국사 모니터링 시스템');
...
begin
  ...
  AccessEventHeaderNameList.Add('일련번호');
  ...
  AccessEventHeaderNameList.Add('출입타입코드');
  AccessEventHeaderNameList.Add('노드번호');
  AccessEventHeaderNameList.Add('기기번호');
  AccessEventHeaderNameList.Add('도어번호');
  AccessEventHeaderNameList.Add('출입승인코드');
  ...
  AlarmEventHeaderNameList.Add(dmFormFunction.GetFormName('0','2','COMMONNODENO'));    //노드번호
...
begin
  ...
  begin
    showmessage( stPrintNo + '에서 이미 사용중입니다.');
    Exit;
  end;
...
begin
  ...
  if Application.MessageBox(pchar(stDoorName + ' 의 카드리더에 전화번호를 재전송합니다. 진행하시겠습니까?'),pchar('Info'),mb_OKCANCEL) = id_cancel then Exit;
  //전화번호 재전송
...
begin
  ...
  if Application.MessageBox(pchar(stDoorName + ' 의 컨트롤러에 출입권한을 재전송합니다. 진행하시겠습니까?'),pchar('Info'),mb_OKCANCEL) = id_cancel then Exit;
  ...
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_HOLIDAYDEVICE_DeviceYearSendStatus(stNodeNo,stEcuID,formatDatetime('yyyy',now),'N'); //공휴일 정보 내려 보내자
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_DOORSCHEDULE_DoorReceive(stNodeNo,stEcuID,'','N');  //스케줄 정보 재전송
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_DEVICERCV_Field_StringValue(stNodeNo,stEcuID,'0','DE_TIMECODEUSE','N');  //타임코드 사용유무 재전송
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_TIMECODEDEVICE_Field_StringValue(stNodeNo,stEcuID,'TC_SEND','N');  //타임코드 재전송
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_DOORRCV_Field_StringValue(stNodeNo,stEcuID,'0','','DO_SETTINGINFO','N');  //출입문 스케줄 사용유무 재전송
  if G_nMonitoringType <> 1 then dmDBUpdate.UpdateTB_DOORRCV_Field_StringValue(stNodeNo,stEcuID,'0','','DO_DOORSCHUSE','N');  //출입문 스케줄 사용유무 재전송
...
begin
  ...
  nHeighCount := FlowPanel2.Height div L_nGroupHeigh ; //세로로 들어가는 갯수
...
begin
  ...
  begin
    ...
    if aData = 'NF' then
    begin
      Cells[L_arrAlarmEventIndexArray[4],1] := 'Server' + inttostr(aDecoderNo) + ' 통신단선';
      RowColor[1] := clRed;
    ...
    begin
      Cells[L_arrAlarmEventIndexArray[4],1] := 'Server' + inttostr(aDecoderNo) + ' 통신복구';
      RowColor[1] := clWhite;
    end;
...
//  Exit;
  ...
  if (sg_AccessEvent.Cells[L_arrAccessEventIndexArray[12],sg_AccessEvent.Row] = 'P')
    OR (sg_AccessEvent.Cells[L_arrAccessEventIndexArray[12],sg_AccessEvent.Row] = 'R')  then  //원격제어
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if G_nMonitoringType = 1 then stTemp:= Cells[L_arrAccessEventIndexArray[3],Row]  //카드일련번호 복사하자.
